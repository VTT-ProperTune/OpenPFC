...
* [x] Implement `kernel/simulation/spectral_etd_system.hpp`: host driver owns work fields in `SimulationState`, transform choreography (forward → filter/N̂ → ETD combine → backward) via `IHostFFT`, `for_each_kpoint` operator preparation, `Etd1Stepper` integration, optional 2/3-rule dealias. Device `IDeviceFFT` instantiation is a remaining M7 slice.
