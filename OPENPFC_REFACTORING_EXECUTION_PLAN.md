...
**M4 is in progress.** `HaloExchange` and `SparseExchange` exist on host and device (device is blocking-only). GPU-aware device transport defaults to pack-to-contiguous + device-pointer MPI. `mpi::communicator::duplicate()` provides opt-in `MPI_Comm_dup`. Consumer migration, the in-repo splitter, and neighbour-agreement opt-out are still ahead. Device-CUDA execution stays on tohtori. M5–M12 have not started.
