...
* [ ] Generalize state: steppers accept any type satisfying the field concepts (`state_concepts.hpp` — wire it in for real) — `Field<double>`, `Field<complex<double>>`, and heterogeneous packs; remove the raw-`std::vector<double>`-only restriction. **All seven single-field leaves take host `Field<double>` via `vec()`. Euler, MultiEuler, RK2/RK3 Heun, and both ETD1 leaves take `Scalar` / `Field<Scalar>`. Vector path remains. ExplicitRK / EmbeddedRK / IMEX still real-only.**
