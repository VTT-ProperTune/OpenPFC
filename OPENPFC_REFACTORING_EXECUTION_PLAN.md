...
* [x] Host `SpectralMeanFieldEtdSystem` + device `DeviceSpectralMeanFieldEtdSystem` on `SimulationState` + `IDeviceFFT` (no model-owned FFTs in the new driver). Host one-step parity vs Gen-1; device vs host toy mean-field ≤1e-10. CPU A/B session `TungstenEtdSession` + binary `tungsten_etd` (JSON domain/time/params/IC/fixed-BC; writers remain Gen-1).
