...
**M4 is in progress.** `halo_geometry.hpp` and host `pfc::comm::HaloExchange<HostSpace>` exist (Faces / Full, blocking + split-phase, optional persistent, multi-field tag blocks). `mpi::communicator::duplicate()` provides opt-in `MPI_Comm_dup`. `gpu_aware_mpi.hpp` decides device-pointer MPI (assume / Open MPI query / Cray `MPICH_GPU_SUPPORT_ENABLED` / optional probe). Device `HaloExchange`, SparseExchange, consumer migration, and the in-repo splitter are still ahead. Device-CUDA work stays on tohtori. M5–M12 have not started.
...
* [x] Create `pfc::comm::HaloExchange<MemorySpace>` in `kernel/decomposition/halo_exchange.hpp` (host) + `runtime/gpu` (device pack/unpack), superseding `PaddedHaloExchanger`/`FullPaddedHaloExchanger`/device twins. Modes: 6-face or 26-direction (for mixed derivatives); blocking `exchange()`; split `start()/finish()`; optional persistent requests (from `halo_persistent.hpp`); multi-field batching (promoted from `apps/kobayashi/src/cuda/kobayashi_batched_halo.hpp`, generalized past its one-MPI-axis corner-fill constraint or failing loudly when unsupported). **Host facade landed** in `comm_halo_exchange.hpp` (`HaloExchange<HostSpace>` composes the existing three host classes; Full has no split-phase; persistent+Full throws). **Device HaloExchange not started** — CUDA not testable on LUMI; HIP device path is remaining M4.
* [ ] Device transport defaults: pack-to-contiguous + device-pointer MPI when GPU-aware (env toggle becomes default per Audit §9); stream-scoped sync instead of `cudaDeviceSynchronize` (`padded_device_halo_exchange.hpp:381`).
...
* [ ] `apps/kobayashi/src/cuda/kobayashi_batched_halo.hpp` (531 lines, promoted) and kobayashi HIP's host-staging workaround.
...
* [ ] Declare `StepAttemptResult`/attempt-commit (from `steppers/step_attempt.hpp`) the single protocol; specify it in `docs/adr/0003-time-integrator-interface.md` (update the existing ADR).
* [ ] Port onto the protocol: `EulerStepper`, `RK2Heun`, `RK3Heun`, `ExplicitRKStepper` (via `euler_attempt.hpp`'s proof path), `EmbeddedRKStepper` (replace reference-member result), `ImexEulerStepper` (drop `ImexStepAttempt`), `Etd1Stepper` (drop `Etd1StepAttempt`/`candidate()` special form).
...
* [ ] Protocol-conformance test template applied to all seven steppers (attempt→reject→attempt→commit sequence; rollback state equality).
