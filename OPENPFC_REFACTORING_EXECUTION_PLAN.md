...
**M4 has started** with `halo_geometry.hpp` (slots, `opposite_slot`, field tag blocks, padded slabs) and `test_halo_geometry.cpp`. The halo-exchanger merge, device transport, Cray GPU-aware probe, SparseExchange, and in-repo splitter are still ahead. M5–M12 have not started. M5 (honest FFT) may proceed in parallel once M3 is treated as code-complete.
...
* [ ] Fix GPU-aware MPI detection for Cray MPICH: runtime self-probe (device-pointer send/recv smoke test à la `verify_gpu_aware_mpi.cpp`) plus `OPENPFC_ASSUME_GPU_AWARE_MPI` override; surface the active mode in the startup log.
