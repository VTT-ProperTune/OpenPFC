...
* [x] `apps/kobayashi/src/cuda/kobayashi_batched_halo.hpp` (531 lines). Removed after Kobayashi CUDA moved onto multi-field `HaloExchange<CUDASpace>`. Host Faces `HaloExchange::exchange()` now posts every bound field then one `MPI_Waitall`. Device Faces still exchanges sequentially per field.
