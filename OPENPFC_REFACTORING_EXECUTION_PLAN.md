...
**M4 is in progress.** `halo_geometry.hpp` and `pfc::comm::HaloExchange` exist on host and device (Faces / Full; host also has split-phase and optional persistent; device is blocking-only). GPU-aware device transport defaults to pack-to-contiguous + device-pointer MPI with stream-scoped sync. `mpi::communicator::duplicate()` provides opt-in `MPI_Comm_dup`. `gpu_aware_mpi.hpp` decides device-pointer MPI. SparseExchange, consumer migration, and the in-repo splitter are still ahead. Device-CUDA execution stays on tohtori. M5–M12 have not started.
