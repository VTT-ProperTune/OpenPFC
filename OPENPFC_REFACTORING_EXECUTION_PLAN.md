...
**M4 is in progress.** `halo_geometry.hpp` and `pfc::comm::HaloExchange` exist on host and device (Faces / Full; host also has split-phase and optional persistent; device is blocking-only). `mpi::communicator::duplicate()` provides opt-in `MPI_Comm_dup`. `gpu_aware_mpi.hpp` decides device-pointer MPI (assume / Open MPI query / Cray `MPICH_GPU_SUPPORT_ENABLED` / optional probe). SparseExchange, consumer migration, and the in-repo splitter are still ahead. Device-CUDA execution stays on tohtori. M5–M12 have not started.
