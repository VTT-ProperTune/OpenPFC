...
* [x] Rebuild `apps/aluminumNew` on `SpectralETDSystem`: physics = temperature-gradient/moving-frame terms + `P_F` kernel + free-energy observable (uses the M7 reduction hook); one `ParameterSchema` replacing the 130-line hand-rolled JSON block (`Aluminum.hpp:359–438`); ETD weights now from the shared cache (its inline `opL/opN` formulas retired — validated against the golden trajectory). **Host:** `MovingFrameMeanFieldETDSystem` + `aluminum_etd` / `AluminumETDSession`. Schema round-trip; operator/`N`/FE pins; one-step / 10-step / \(G_{\mathrm{grid}}\) vs Gen-1 ≤1e-10. Device session not yet. Gen-1 still built.
