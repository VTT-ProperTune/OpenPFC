...
World::World(const Int3 &dimensions) : World(dimensions, {0.0, 0.0, 0.0},
{1.0, 1.0, 1.0}) {}
*/
...
World::Real3 World::physical_coordinates(const Int3 &indices) const noexcept {
  return {m_origin[0] + indices[0] * m_spacing[0],
          m_origin[1] + indices[1] * m_spacing[1],
...
bool World::operator==(const World &other) const noexcept {
  return m_size == other.m_size && m_origin == other.m_origin &&
...
std::ostream &operator<<(std::ostream &os, const World &w) noexcept {
  os << std::fixed
     << std::setprecision(2); // Set fixed-point notation and precision
  os << "(size = {" << w.m_size[0] << ", " << w.m_size[1] << ", " << w.m_size[2]
     << "}";
  os << ", origin = {" << w.m_origin[0] << ", " << w.m_origin[1] << ", "
     << w.m_origin[2] << "}";
