...
int World::total_size() const noexcept { return m_size[0] * m_size[1] * m_size[2]; }
...
bool World::operator!=(const World &other) const noexcept { return !(*this == other); }
...
World create_world(const World::Int3 &dimensions, const World::Real3 &origin, const World::Real3 &spacing) {
  ...
  return World(dimensions, origin, spacing);
}

World create_world(const World::Int3 &dimensions) { return create_world(dimensions, {0.0, 0.0, 0.0}, {1.0, 1.0, 1.0}); }

World::Int3 get_size(const World &world) noexcept { return world.get_size(); }
size_t get_size(const World &world, int dim) noexcept { return world.get_size()[dim]; }
World::Real3 get_origin(const World &world) noexcept { return world.get_origin(); }
double get_origin(const World &world, int idx) noexcept { return world.get_origin()[idx]; }
World::Real3 get_spacing(const World &world) noexcept { return world.get_spacing(); }
double get_spacing(const World &world, int idx) noexcept { return world.get_spacing()[idx]; }
