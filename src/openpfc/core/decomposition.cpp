...
Decomposition::Decomposition(const World &world,
                             const heffte::box3d<int> &inbox,
...
std::ostream &operator<<(std::ostream &os, const Decomposition &) {
  ...
  d.proc_grid[1] << ", " << d.proc_grid[2]
     << "])\n";
  */
  // os << "Domain in real space: [" << w.size()[0] << ", " << w.size()[1] << ",
  // " << w.size()[2] << "]\n"; os << "Domain in complex space: [" << d.Lx_c <<
  // ", " << d.Ly_c << ", " << d.Lz_c << "]\n";
  /*
  ...
  << in.count() << " indexes) => "; os << "[" << out.low[0] << ", " <<
  out.low[1] << ", " << out.low[2] << "] x "; os << "[" << out.high[0] << ", "
  << out.high[1] << ", " << out.high[2] << "] (" << out.count() << "
  indexes)\n";
  }
 */
  return os;
