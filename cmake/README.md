...
On Tohtori, Cursor / VS Code CMake Tools may configure without `module load`, so MPI and the matching GCC are missing from the environment. Use CMake presets `tohtori-debug` and `tohtori-release` in the root `CMakePresets.json`: they set `CMAKE_TOOLCHAIN_FILE` to the historically named `cmake/toolchains/tohtori-gcc11-openmpi.cmake` and preset `PATH` and `LD_LIBRARY_PATH` for the current GCC 15.2.0 + Open MPI 5.0.10 site stack. For a custom stack, set both `OPENPFC_GCC_ROOT` and `OPENMPI_ROOT`. Build directories are `builds/tohtori-debug` and `builds/tohtori-release`.

The toolchain sets `MPI_C_COMPILER` / `MPI_CXX_COMPILER` to Open MPI wrappers under `/share/apps/OpenMPI/5.0.10` when `OPENMPI_ROOT` is unset (Slurm PMI/`srun`). Export the compiler and MPI roots together when your cluster layout differs. Presets `tohtori-debug` / `tohtori-release` set `OpenPFC_ENABLE_CODE_COVERAGE=OFF` so configure stays quiet on nodes without `lcov`; re-enable with `-DOpenPFC_ENABLE_CODE_COVERAGE=ON` after installing `lcov`.
