 ...
 * @brief Single Fourier-mode height: \(u=u_0+A\cos(2\pi n_x x/L_x+\cdots)\).
 *
...
public:
  ...
  void apply(pfc::field::FieldOutput<double> field, const pfc::Domain &domain,
    ...
    pfc::field::apply(field, domain, box, [=](const pfc::Real3 &x) {
      return u0 + amp * std::cos(kx * x[0] + ky * x[1] + kz * x[2]);
    });
  }
...
inline void from_json(const nlohmann::json &j, CosineMode &ic) {
  ...
  if (j.contains("u0")) {
    if (!j["u0"].is_number()) {
      throw std::invalid_argument("cosine_mode: 'u0' must be numeric.");
    }
    ic.set_u0(j["u0"].get<double>());
  } else if (j.contains("h0") && j["h0"].is_number()) {
    ic.set_u0(j["h0"].get<double>());
  }
