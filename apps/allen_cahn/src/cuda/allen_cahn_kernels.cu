...
__global__ void allen_cahn_step_kernel(double *core, const double *hpx,
                                       const double *hnx, const double *hpy,
                                       const double *hny, const double * /*hpz*/,
                                       const double * /*hnz*/, int nx, int ny,
                                       int nz, int hw, double inv_dx2,
  ...
  const double uyp =
      (iy + 1 < ny)
          ...
          : hpy[static_cast<std::size_t>(iz) * static_cast<std::size_t>(nx * hw) +
                static_cast<std::size_t>(ix)];
  const double dxx = uxp + uxm - 2.0 * p;
  const double dyy = uyp + uym - 2.0 * p;
  const double lap = inv_dx2 * dxx + inv_dy2 * dyy;
  core[c] = p + dt * (M * lap - inv_eps2 * (p * p * p - p) + driving_force);
}
...
void allen_cahn_step_cuda(double *core_dev, const double *hpx_dev,
  ...
  allen_cahn_step_kernel<<<grid, block>>>(
      ...
      inv_dx2, inv_dy2, dt, M, inv_eps2, driving_force);
