...
#error "kobayashi/device_step_cuda.hpp requires CUDA (configure with -DOpenPFC_ENABLE_CUDA=ON)"
#endif

namespace kobayashi {

/// Stage A: Laplacians, gradients, and anisotropic \(\epsilon(\theta)\) (matches CPU/HIP ordering).
...
/**
 * @brief Apply **periodic** x/y (and z for `nz==1`) halos on device for `hw == 1` only.
 *
