...
| [**`inverse_homogenization/`**](inverse_homogenization/README.md) | **Periodic inverse homogenization** (target \(C_H\); issue #161). Explicit sixteenth-app exception: PDE-constrained inverse design, not another PDE demo. Forward \(C_H\) and adjoint sensitivity are in tree; the inverse loop is later. | CLI (`--key=value`) | Reuses `openpfc_apps/microelasticity.hpp` (six imposed-strain FFT solves); CPU |
