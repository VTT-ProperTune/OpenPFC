...
CMake wires subdirectories from [`apps/CMakeLists.txt`](CMakeLists.txt) in this order: `tungsten`, `aluminumNew`, `cahn_hilliard`, `thin_film`, `surface_diffusion`, `kawahara`, `ehd_film`, `gradient_elasticity`, `higher_order_pfc`, `inverse_homogenization`, `allen_cahn`, `alloy_dendrite_elastic`, `heat3d`, `wave2d`, `kobayashi`, `vlasov_maxwell`.
...
| [**`inverse_homogenization/`**](inverse_homogenization/README.md) | **Periodic inverse homogenization** (target \(C_H\); issue #161). Explicit sixteenth-app exception: PDE-constrained inverse design, not another PDE demo. Forward \(C_H\), adjoint, Allen–Cahn inverse loop, CH process family, manufacturability, and 3-D cells are in tree. Catalog stays closed after this. | CLI (`--key=value`) | Reuses `openpfc_apps/microelasticity.hpp` (six imposed-strain FFT solves); CPU + optional HIP Green |
