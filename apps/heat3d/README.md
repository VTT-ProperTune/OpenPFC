...
| **`heat3d_fd`** | Compact FD (orders 2–20) | The production path. **Two `pfc::field::PaddedBrick<double>` bricks** (`u`, `du`), `pfc::communication::PaddedHaloExchanger<double> halo(u, MPI_COMM_WORLD)` and `pfc::gradient::FDGradient<HeatGrads> grad(u, fd_order)` — both bound to `u`. Each step is `pfc::communication::exchange(halo)` / `pfc::field::for_each(du, …) / u += dt * du` (with `du[idx]` writes). For overlap, `pfc::communication::start_exchange` / `finish_exchange` are the split API (also injected into `pfc::` as `pfc::start_exchange` / `pfc::finish_exchange` for existing call sites). For an FFT-safe unpadded core + separated face halos, tests use `pfc::sim::stacks::FdCpuStack` (which still bundles those concerns through `pfc::sim::DuField`). |
...
- **[`include/heat3d/spectral_heat_propagator.hpp`](include/heat3d/spectral_heat_propagator.hpp)** — heat-specific **implicit-Euler-in-Fourier-space** propagator. Builds the `1 / (1 - dt·D·k²)` symbol table once from the FFT layout and exposes `step(pfc::data::Field<double>&)`. Backend-agnostic (takes `pfc::fft::IFFT&`).
...
- **[`src/cpu/heat3d_fd_scratch.cpp`](src/cpu/heat3d_fd_scratch.cpp)** — **version 0**, the from-scratch driver. Inlines the IC `exp(-r²/(4 D))` and the Laplacian; runs three nested `for (k) for (j) for (i)` loops with manual padded `lin = (i+hw)*sx + (j+hw)*sy + (k+hw)*sz` and stencil neighbours `u_ptr[lin ± sx/sy/sz]`; per-step Laplacian held in a `std::vector<double>` of size `nx*ny*nz` with no halo (visibly different flat indexing from the padded `u`). The only OpenPFC pieces in the hot loop are `pfc::PaddedHaloExchanger<double>::exchange_halos`, plus the `PaddedBrick<double>` ctor for storage and `domain::create / decomposition::create` for geometry. Read this one first.
- **[`src/cpu/heat3d_fd.cpp`](src/cpu/heat3d_fd.cpp)** — compact FD driver. After constructing two `pfc::field::PaddedBrick<double>` bricks (`u`, `du`) plus `pfc::communication::PaddedHaloExchanger<double> halo(u, MPI_COMM_WORLD)` and `pfc::gradient::FDGradient<HeatGrads> grad(u, cfg.fd_order)`, the hot loop reads:
  ...
  Halo, gradient evaluator, and per-cell sweep are three visible primitives composed by the user. `pfc::communication::exchange` is the blocking one-shot wrapper; `pfc::gradient::evaluate(grad, idx)` keeps the inner loop dimension-agnostic; `PaddedBrick::operator[]` matches `operator()(Int3)`. `PaddedBrick::operator+=(ScaledField)` does the axpy. The spectral twin still bundles those concerns through `pfc::sim::DuField`.
...
- **[`tests/test_heat3d.cpp`](tests/test_heat3d.cpp)** — Catch2 unit tests covering: `heat3d::kD` is pinned to 1.0; `HeatModel` (default IC at the origin, IC override, the `rhs = kD * (xx + yy + zz)` formula); the slim per-binary CLI parsers (happy paths + every rejection case); the analytic reference solution; two single-rank integration tests against `pfc::data::Field<double>` / `EulerStepper`; **two** single-rank cases for the manual driver (smoke + L2-vs-analytic, and parity vs the compact `FdCpuStack` path); and **two** single-rank cases for the from-scratch driver (smoke + L2-vs-analytic, and parity vs the compact path to within 1e-7). Built into the `test_heat3d` executable and registered with CTest as `heat3d-all-tests` whenever `OpenPFC_BUILD_TESTS=ON` and Catch2 is available (set `HEAT3D_ENABLE_TESTS=OFF` to skip).

### Where OpenMP runs (FD)

In **`heat3d_fd.cpp`** the per-step sweep over `du` is delegated to `pfc::field::for_each` (`include/openpfc/kernel/field/brick_iteration.hpp`), which iterates the brick's owned cells in cache-friendly k-outer / j-middle / i-inner order and yields a `pfc::Int3` to the user lambda. Each cell evaluates `pfc::gradient::evaluate(grad, idx)` against `pfc::gradient::FDGradient<HeatGrads>` (per-axis stencil application from `kernel/field/fd_apply.hpp`), writes the per-point residual into `du[idx]`, and the explicit-Euler step `u += dt * du` then updates the field in place.

In **`heat3d_fd_manual.cpp`** the same parallelisation is applied explicitly: `pfc::field::for_each_inner_omp` parallelises the inner-region stencil over `(k, j)`, `pfc::field::for_each_owned_omp` parallelises the Euler update; the (cheap) border pass stays serial. **Not parallel** in either driver: the MPI halo exchange (`pfc::SparseHaloExchanger` on separated-layout stacks elsewhere; **`pfc::communication::PaddedHaloExchanger`** in these heat3d FD binaries — still spellable as `pfc::PaddedHaloExchanger` via a compatibility `using`).
