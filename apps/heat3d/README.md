...
- **[`src/cpu/heat3d.cpp`](src/cpu/heat3d.cpp)** — the **driver**. Pure orchestration: three `run_*` entry points (`run_fd`, `run_spectral`, `run_spectral_pointwise`) and a one-statement `main` that delegates MPI lifecycle to `pfc::runtime::mpi_main`. Wires `HeatModel` and `SpectralHeatPropagator` to the OpenPFC primitives (`pfc::field::LocalField`, `pfc::field::FdGradient`, `pfc::field::SpectralGradient`, `pfc::sim::steppers::EulerStepper`).
