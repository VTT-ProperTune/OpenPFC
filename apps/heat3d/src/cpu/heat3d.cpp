 ...
 * L2 reporting    | `heat3d/reporting.hpp`                     | | Point-wise
 ...
 * entry-point wrapper          | `pfc::runtime::mpi_main`                   | |
 * Single-rank CPU-affinity rescue      |
 * `pfc::runtime::reset_cpu_affinity_if_single_mpi_rank` |
 *
...
void run_fd(const RunConfig &cfg, int rank, int nproc) {
  ...
  auto u = field::LocalField<double>::from_subdomain(decomp, rank, hw);
...
void run_spectral(const RunConfig &cfg, int rank, int nproc) {
  ...
  auto u = field::LocalField<double>::from_inbox(decomposition::get_world(decomp),
  ...
  heat3d::SpectralHeatPropagator prop(fft, u, model.D, cfg.dt);

  runtime::MpiTimer timer{MPI_COMM_WORLD};
  runtime::tic(timer);
  for (int step = 0; step < cfg.n_steps; ++step) prop.step(u);
  const double max_elapsed = runtime::toc(timer);
...
void run_spectral_pointwise(const RunConfig &cfg, int rank, int nproc) {
  ...
  runtime::tic(timer);
  double t = 0.0;
  for (int step = 0; step < cfg.n_steps; ++step) t = stepper.step(t, u.vec());
  const double max_elapsed = runtime::toc(timer);
