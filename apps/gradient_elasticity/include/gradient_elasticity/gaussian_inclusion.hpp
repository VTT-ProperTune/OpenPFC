...
/**
 * @file gaussian_inclusion.hpp
 * @brief Periodic 2-D Gaussian eigenstrain \(g=g_0+A\exp(-r^2/(2\sigma^2))\).
 *
 * JSON `"type": "gaussian_inclusion"`. Default centre is the domain midpoint.
 */
...
class GaussianInclusion : public pfc::FieldModifier {
public:
  ...
  const std::string &get_modifier_name() const override {
    static const std::string k{"GaussianInclusion"};
    return k;
  }
  ...
  void apply(pfc::field::FieldOutput<double> field, const pfc::Domain &domain,
    ...
    if (!(sig > 0.0)) {
      throw std::invalid_argument("gaussian_inclusion: sigma must be > 0.");
    }
    ...
    pfc::field::apply(field, domain, box, [=](const pfc::Real3 &x) {
      ...
      return g0 + amp * std::exp(-(dx * dx + dy * dy) * inv);
    });
  }
...
inline void from_json(const nlohmann::json &j, GaussianInclusion &ic) {
  pfc::ui::detail::throw_unless_json_modifier_type(
      j, "gaussian_inclusion",
      "Invalid JSON input: missing or incorrect 'type' field.");
  if (j.contains("g0")) {
    if (!j["g0"].is_number()) {
      throw std::invalid_argument("gaussian_inclusion: 'g0' must be numeric.");
    }
    ic.set_g0(j["g0"].get<double>());
  }
  if (!j.contains("amplitude") || !j["amplitude"].is_number()) {
    throw std::invalid_argument(
        "gaussian_inclusion: missing or invalid 'amplitude' field.");
  }
  ic.set_amplitude(j["amplitude"].get<double>());
  if (!j.contains("sigma") || !j["sigma"].is_number()) {
    throw std::invalid_argument(
        "gaussian_inclusion: missing or invalid 'sigma' field.");
  }
  ic.set_sigma(j["sigma"].get<double>());
  if (j.contains("x0")) {
    if (!j["x0"].is_number()) {
      throw std::invalid_argument("gaussian_inclusion: 'x0' must be numeric.");
    }
    ic.set_x0(j["x0"].get<double>());
  }
  if (j.contains("y0")) {
    if (!j["y0"].is_number()) {
      throw std::invalid_argument("gaussian_inclusion: 'y0' must be numeric.");
    }
