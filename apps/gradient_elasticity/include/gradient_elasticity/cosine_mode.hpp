 ...
 * @brief Single Fourier-mode inclusion: \(g=g_0+A\cos(2\pi n_x x/L_x+\cdots)\).
 *
...
public:
  ...
  void apply(pfc::field::FieldOutput<double> field, const pfc::Domain &domain,
    ...
    pfc::field::apply(field, domain, box, [=](const pfc::Real3 &x) {
      return g0 + amp * std::cos(kx * x[0] + ky * x[1] + kz * x[2]);
    });
  }
...
inline void from_json(const nlohmann::json &j, CosineMode &ic) {
  ...
  if (!j.contains("g0") || !j["g0"].is_number()) {
    throw std::invalid_argument("cosine_mode: missing or invalid 'g0' field.");
  }
