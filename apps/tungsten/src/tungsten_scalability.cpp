...
public:
  ...
  void run_hip_test_impl(int size_x, int size_y, int size_z, int num_iterations,
    ...
    try {
      ...
      auto size_inbox = hip_fft.size_inbox();
      auto size_outbox = hip_fft.size_outbox();
      double memory_mb =
          (size_inbox * sizeof(RealType) * 3 + size_outbox * sizeof(RealType) * 3 +
           size_outbox * sizeof(std::complex<RealType>) * 3) /
          (1024.0 * 1024.0);
      ...
      if (rank == 0) {
        ...
        save_result(result);
        std::cout << "  HIP (" << precision_name << "): " << time_per_iter * 1000
                  << " ms/iteration"
                  << " (FFT: " << fft_time_per_iter * 1000
                  << " ms, Other: " << other_time_per_iter * 1000
                  << " ms, Setup: " << setup_time * 1000 << " ms)" << '\n';
  ...
  void run_hip_test(int size_x, int size_y, int size_z, int num_iterations) {
    run_hip_test_impl<double>(size_x, size_y, size_z, num_iterations, "double");
    run_hip_test_impl<float>(size_x, size_y, size_z, num_iterations, "float");
  }
#endif
