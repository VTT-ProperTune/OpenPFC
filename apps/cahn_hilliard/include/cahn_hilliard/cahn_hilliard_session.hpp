 ...
 * `register_catalog()` adds the `cosine_mode` initial condition. HIP builds
...
class CahnHilliardSession
    : public pfc::ui::SpectralETDSession<CahnHilliardPhysics<double, pfc::HostSpace>,
                                         pfc::sim::stacks::SpectralCPUStack> {
public:
  using Base =
      pfc::ui::SpectralETDSession<CahnHilliardPhysics<double, pfc::HostSpace>,
                                  pfc::sim::stacks::SpectralCPUStack>;

  CahnHilliardSession(const nlohmann::json &settings, int rank, int nproc,
                      MPI_Comm comm = MPI_COMM_WORLD)
      : Base(settings, rank, nproc, comm, etd_options()) {}
};

#if defined(OpenPFC_ENABLE_HIP_SPECTRAL)
class CahnHilliardHIPSession
    : public pfc::ui::SpectralETDSession<
          CahnHilliardPhysics<double, pfc::HIPSpace>,
          pfc::sim::stacks::GPUSpectralStack<pfc::HIPSpace>> {
public:
  using Base =
      pfc::ui::SpectralETDSession<CahnHilliardPhysics<double, pfc::HIPSpace>,
                                  pfc::sim::stacks::GPUSpectralStack<pfc::HIPSpace>>;

  CahnHilliardHIPSession(const nlohmann::json &settings, int rank, int nproc,
                         MPI_Comm comm = MPI_COMM_WORLD)
      : Base(settings, rank, nproc, comm, etd_options()) {}
};
static_assert(
