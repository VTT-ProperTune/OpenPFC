...
int run_cost(const CostConfig &cfg, int rank, int nproc, MPI_Comm comm) {
  ...
  const double dt_lim =
      alloy_dendrite::explicit_dt_limit(p, cfg.dx, 3, cfg.fd_order);
  ...
  // Same inputs `ElasticCoupling` would build, so the timed host solve is
  // the science adapter's solve rather than a private re-derivation of it.
  ...
  ep.eps_T = cfg.eps_T;
  ep.U_ref = cfg.u_ref;
  ep.scheme = MicroelasticityScheme::EyreMilton;
