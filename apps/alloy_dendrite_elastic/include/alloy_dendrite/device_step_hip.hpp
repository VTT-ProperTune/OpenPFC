...
struct DeviceParams {
  ...
  double D_th{0.0};
  double M_c{0.0};
  double eps4{0.0}; ///< Only the `eps4 == 0` early-out reads this.
  /// `a_s = aniso_amp (1 + aniso_slope * sum n_i^4)`, and `aniso_ampslope`
  /// is their product, which is what the flux prefactor needs. All three are
  /// formed on the host from `ModelParams::aniso_form` so the device never
  /// re-derives them and the two paths share the same constants bit for bit.
  double aniso_amp{1.0};
  double aniso_slope{0.0};
  double aniso_ampslope{0.0};
...
struct DeviceFields {
  ...
  /// only on owned cells, so the caller need not exchange its halo.
