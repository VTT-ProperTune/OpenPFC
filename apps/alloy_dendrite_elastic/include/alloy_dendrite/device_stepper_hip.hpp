...
private:
  ...
  void build_params_() {
    ...
    m_dp.M_c = m_p.M_c;
    m_dp.eps4 = m_p.eps4;
    // Same two constants the host evaluator forms in `evaluate_anisotropy`,
    // and their product, so both paths multiply identical doubles rather
    // than each deriving them from `eps4` in its own order.
    const bool kr = (m_p.aniso_form == AnisotropyForm::KarmaRappel);
    m_dp.aniso_amp = kr ? (1.0 - 3.0 * m_p.eps4) : 1.0;
    m_dp.aniso_slope = kr ? (4.0 * m_p.eps4 / (1.0 - 3.0 * m_p.eps4)) : m_p.eps4;
    m_dp.aniso_ampslope = m_dp.aniso_amp * m_dp.aniso_slope;
