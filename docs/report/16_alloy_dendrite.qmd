...
|---|---|---|---|---|---|
| `fta-aligned` | $0$ | $(24, 102.4)$ | pending | pending | pending |
| `fta-misori` | $0.2\,\mathrm{rad}$ | same | pending | pending | pending |
| `fta-bicrystal` | $\pm 0.2\,\mathrm{rad}$ | $(24, 64)$, $(24, 140.8)$ | pending | pending | pending |

: FTA directional campaign on LUMI-C `standard`, $640\times 256$, $\Delta x=0.8\,W_0$, $t_{\mathrm{end}}=1200$, $G^{*}=0.02$, $V_p=0.05$, $M_c=0.5$, $\epsilon_4=0.04$. Numbers are filled from the job summary once it finishes; until then this table is a recipe, not a result. {#tbl-ad-fta}
...
9. **FTA is infinite-Le Bridgman, not a metallic Lewis-number run.**
   ...
   pending the LUMI-C job; a missing job ID means the table is a recipe.
