...
|---|---|---|---|---|---|
| `fta-aligned` | $0$ | $(24, 102.4)$ | pending | pending | 21948410 |
| `fta-misori` | $0.2\,\mathrm{rad}$ | same | pending | pending | 21948410 |
| `fta-bicrystal` | $\pm 0.2\,\mathrm{rad}$ | $(24, 64)$, $(24, 140.8)$ | pending | pending | 21948410 |

: FTA directional campaign on LUMI-C `standard`, $640\times 256$, $\Delta x=0.8\,W_0$, $t_{\mathrm{end}}=1200$, $G^{*}=0.02$, $V_p=0.05$, $M_c=0.5$, $\epsilon_4=0.04$. Submitted as job 21948410; $V_{\mathrm{tip}}$ is filled from the summary CSV once it finishes. Until then this table is a recipe, not a result. {#tbl-ad-fta}

The science questions the table is built to answer, once the numbers
exist: does a misoriented grain grow slower than an aligned
$\langle 100\rangle$ grain under the Bridgman field, and does the
aligned grain's advantage show up as a lag of $V_{\mathrm{rel}}$? The
bicrystal is two equally misoriented grains competing in one $\phi$,
not an aligned-versus-misoriented pair and not a grain-boundary energy
minimum. Figures use a **downstream crop**, not a central crop of the
cell (`make_alloy_dendrite_figures.py --fta-dir`).
...
9. **FTA is infinite-Le Bridgman, not a metallic Lewis-number run.**
   ...
   pending LUMI-C job 21948410; $V_{\mathrm{tip}}$ is not a result
   until that job writes the summary.
