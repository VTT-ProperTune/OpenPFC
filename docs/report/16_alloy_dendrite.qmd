...
Every other continuum application in this report solves one physics with one
...
solve, coupled, in one application.** The kinetic chapter that follows is a
different kind of exception: its unknown does not live on the grid the
fields live on.
...
**At the default liquid-shear regularisation ($\mu_l/\mu_s = 0.05$) the
elastic energy of the solutal misfit slows the dendrite tip by 16.7 % and
fattens it by 11.0 %.** That is the effect at a conventionally chosen
regularisation, not a quantitatively converged alloy prediction;
@sec-ad-sensitivity scans the regularisation, and the magnitude falls with
it. What the ladder below is for is the qualitative structure: signed,
monotone in the coupling, and a slide along the solvability curve rather
than a change of selection.
...
an effect of 16.7 % at this regularisation: a factor of 240. The iteration count rising with $N$ is
...
−16.7 % and $\rho$ by +11.0 % at this regularisation. That is not a coincidence and it is the most
...
17 % at the default regularisation. Clamping the cell at zero mean strain stores $2.1\times$ the energy,
...
quoted in @tbl-ad-science is the effect *at a conventionally chosen
regularisation*, and a substantial part of it is the regularisation rather
than the alloy. A reader should take from that table that the coupling is
real, signed, monotone and large enough to matter, and should not take
...
## Directional solidification {#sec-ad-fta}

Issue #85 asked for a Stage-4 directional case,
$T(z,t) = T_0 + G(z - V_p t)$, and competing dendrites. That geometry
arrived first as a separate application in unmerged PR #103
(`alloy_pf_directional`, Pinomaa et al., *J. Crystal Growth* 2020,
Al–Cu FTA bicrystal). It was not merged: the catalog is closed at
fifteen. The leftover is here, on `alloy_dendrite_growth`, not as a
sixteenth binary. Issue #154 put the geometry on this binary; issue
#155 is the campaign that runs it.

The frozen-temperature field is the dimensionless Bridgman profile

$$
\theta(x,t) = \frac{G}{\Delta T_h}\,(x - x_0 - V_p t).
$$ {#eq-ad-fta}

`--gradient` is $G/\Delta T_h$ in $1/W_0$, `--pulling` is $V_p$ in
$W_0/\tau_0$. Equation (4) is **not** integrated (`evolve_theta` stays
off, the thermal Laplacian is skipped); $\theta$ is **not** zeroed;
$M_c\theta$ in @eq-ad-phi stays live. This is **infinite-Le FTA**, not
a metallic evolving-$\theta$ Lewis-number run. Cubic anisotropy is
evaluated on $\mathbf{n}' = R(\theta_c)^\mathsf{T}\mathbf{n}$, then
$\sum n_i'^4$, with the same normalised Karma–Rappel $a_s$ as the rest
of this chapter. A second tanh seed (`--seed2-*`, `--crystal-angle2`) is
a single-$\phi$ bicrystal. There is no grain-boundary energy and no
second order parameter; two solids that meet merge. Do not present a
groove as a GB energy minimum. The tip diagnostic for FTA is the
downstream solid-to-liquid crossing, not the $+x$ arm from the box
centre; a bicrystal reports two tips and the geometric groove between
them. `--elastic` is **off** for this campaign.

**Elevated-$G$ directional geometry, not an Al–Cu furnace.**
`--gradient=0.02` means $\ell_T = 50\,W_0$; `--pulling=0.05` means
$\ell_D = D_l/V_p = 40\,W_0$. $\theta$ is scaled by the hypercooling
$L/c_p \approx 336\,\mathrm{K}$ (@tbl-ad-material), not by the freezing
range $\Delta T_0$. A metal Bridgman gradient of $\sim 10\,\mathrm{K\,mm^{-1}}$
would make $\ell_T$ tens of millimetres against a $512\times 205\,W_0$
box. The campaign does not claim a CMS 2026 / Pinomaa $12\times 3.2\,\mu\mathrm{m}$
replication.

| Case | $\theta_c$ | Seeds | $V_{\mathrm{tip}}$ | $V_{\mathrm{rel}}=V_{\mathrm{tip}}-V_p$ | Job |
|---|---|---|---|---|---|
| `fta-aligned` | $0$ | $(24, 102.4)$ | diverged $t\approx 27$; not a result | — | 21948410 |
| `fta-misori` | $0.2\,\mathrm{rad}$ | same | diverged $t\approx 27$; not a result | — | 21948410 |
| `fta-bicrystal` | $\pm 0.2\,\mathrm{rad}$ | $(24, 64)$, $(24, 140.8)$ | diverged $t\approx 27$; not a result | — | 21948410 |

: FTA directional campaign on LUMI-C `standard`, $640\times 256$, $\Delta x=0.8\,W_0$, $t_{\mathrm{end}}=1200$, $G^{*}=0.02$, $V_p=0.05$, $M_c=0.5$, $\epsilon_4=0.04$. Job 21948410 ran the recipe and is **not** a $V_{\mathrm{tip}}$ result: every case left $\phi$ unbounded by $t\approx 27$. A 1-rank probe of the same flags dies at the same time. The periodic box carries a $\theta$ jump $G L_x = 10.2$ hypercoolings at the seam; that is a limitation of periodic FTA, not a measured growth rate. {#tbl-ad-fta}

The science questions the table is built to answer, once a *valid* run
exists: does a misoriented grain grow slower than an aligned
$\langle 100\rangle$ grain under the Bridgman field, and does the
aligned grain's advantage show up as a lag of $V_{\mathrm{rel}}$?
Job 21948410 does not answer them. The bicrystal is two equally
misoriented grains competing in one $\phi$, not an
aligned-versus-misoriented pair and not a grain-boundary energy
minimum. Figures use a **downstream crop**, not a central crop of the
cell (`make_alloy_dendrite_figures.py --fta-dir`), and wait on a valid
fields directory.
...
7. **Coupled elasticity is host-side.** The device twin of the
   finite-difference step is in the tree and has a CPU/GPU parity test; the
   elastic solve is still host-side. Measurements on a GPU build put the
   host round-trip at 0.3 % of the coupled step but the host *solve* at
   500–3000 times the device step, so a GPU coupled run is not currently
   worth doing. A device Green operator over rocFFT is the obvious next
   step and is not in this application.
...
9. **FTA is infinite-Le Bridgman, not a metallic Lewis-number run.**
   The campaign in @sec-ad-fta / @tbl-ad-fta imposes
   $\theta=(G/\Delta T_h)(x-x_0-V_p t)$ with `evolve_theta=0`. It does
   not evolve latent heat, does not claim a grain-boundary energy (one
   $\phi$; solids that meet merge), and is elevated-$G$ directional
   geometry rather than an Al–Cu furnace. Numbers in @tbl-ad-fta are
   pending a *valid* run. Job 21948410 wrote a summary with
   `valid=0` and is not a $V_{\mathrm{tip}}$; the periodic $\theta$
   jump $G L_x = 10.2$ at the seam is the suspected cause.
10. **No paper-scale melt-pool / CMS 2026 / Pinomaa $12\times 3.2\,\mu\mathrm{m}$
    replication.** That remains unattempted. Do not read @sec-ad-fta as
    one. Periodic boundaries are still periodic — there is no no-flux
    mould wall from PR #103.

## Why this one is harder than the others

Every other continuum application in this report has one field family and one
...
ladder whose rungs are not interchangeable. The kinetic chapter that follows
is a different kind of exception and is not this one.
