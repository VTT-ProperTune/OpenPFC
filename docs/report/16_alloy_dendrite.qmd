...
informative number in the table: solvability theory says $\sigma^{*}$ is set
...
pair, $V\rho$ falls 5.3 %, which inverting the 2-D Ivantsov relation makes an
...
#### Thermo-solutal: the two misfits partly cancel
...
**The liquid shear modulus is the dominant systematic, and it does not have
a clean limit.** A liquid supports no shear; the solve cannot take zero,
because two channels of the local stiffness become singular and every fixed
point in this family then has contraction factor one. $\mu_l/\mu_s$ is
therefore a regularisation, and the effect it regularises is not small: the
measured slowdown runs 9.8 %, 16.7 %, 23.1 % at $\mu_l/\mu_s = 0.02$, 0.05,
0.10, roughly logarithmic in $\mu_l$ and with no sign of settling over the
range a solve can afford. **The magnitude of the Stage-4 result should
therefore be read as "of order 10–20 % at a physically defensible
regularisation", not as $16.7 \pm 0.1$ %.** What is *not* sensitive to it is
the sign, the monotonicity in $\lambda_{\mathrm{el}}$, the near-invariance of
$\sigma^{*}$, and the partial cancellation of the two eigenstrains — all of
which hold across the whole scan.
...
4. **The liquid shear regularisation sets the magnitude**, as above.
