...
$\langle 100 \rangle$ cross. **The coupled $512^3$ run demonstrates that the
machinery works at LUMI scale. It is not, by itself, a 3-D dendrite science
result; the quantitative selection claims in this chapter remain 2-D.**
...
A longer isothermal $256^3$ run (elastic off, $\epsilon_4 = 0.05$, job
21981928) is the morphology that $t=150$ does not reach. @fig-ad-3d-long
is $t = 73$ (still a cross) against $t = 165$ (parabolic $\langle 100\rangle$
tips, grooves between arms, $V = 0.488$, $\rho = 5.23$, arm length
$\approx 87\,W_0$). The three mid-planes in @fig-ad-3d-long-slices agree,
as cubic symmetry requires. There is no noise, so there are no
sidebranches — that is still a missing 3-D claim, not a failure of this
run.

![Isothermal $256^3$ dendrite, $\epsilon_4=0.05$. Left: $t=73$, a rounded
cross. Right: $t=165$, developed $\langle 100\rangle$ arms. Job
21981928.](figures/alloy_dendrite_3d_long_growth.png){#fig-ad-3d-long}

![Same run, $t=165$: $xy$, $xz$ and $yz$ mid-planes. Cubic symmetry, not
an extruded 2-D crystal.](figures/alloy_dendrite_3d_long_slices.png){#fig-ad-3d-long-slices}

{{< pagebreak >}}

![Mid-$z$ plane of the $256^3$ isothermal dendrite at $t=165$. $V=0.488$,
$\rho=5.23$. Job 21981928.](figures/alloy_dendrite_3d_long_mid.png){#fig-ad-3d-long-mid}
...
: The Stage-4 effect against the liquid-shear regularisation, over a 200-fold range. Every run converged; none hit the iteration cap. Exploratory: no job IDs in this repository. {#tbl-ad-mu}

A later evaluation-grade scan on a smaller box does **not** reproduce
the 16.7% row. Jobs **21995259** (eight ratios, \(256^2\),
\(t_{\mathrm{end}}=200\)) and **21996493** (the same box at
\(\mu_l/\mu_s=0.001\) with `--el-iter=400`) give a few-percent
slowdown where the Green solve converges, and **−2.92%** at the
qualified soft point (`el_nonconverged=0`, max 107 iterations). CSVs:
`docs/report/data/dendrite_shear_21995259.csv` and
`dendrite_shear_21996493.csv`. That vehicle is not @tbl-ad-science.
Do not mix the two boxes.
...
$M_c\theta$ in @eq-ad-phi stays live. Cubic anisotropy is evaluated on
$\mathbf{n}' = R(\theta_c)^\mathsf{T}\mathbf{n}$, then
...
second order parameter; two solids that meet merge. The tip diagnostic
for FTA is the downstream solid-to-liquid crossing, not the $+x$ arm
from the box centre; a bicrystal reports two tips and the geometric
groove between them. `--elastic=1` still builds the eigenstrain from
the imposed $U$ and $\theta$.

**No paper-scale melt-pool result is claimed.** The CMS 2026
reduced-scale replication was not run. The 12×3.2 µm Al–Cu bicrystal of
PR #103 was not re-run on this binary. What is in the tree is the
geometry, the crystal-frame anisotropy, the conserved two-seed initial
condition, and a cheap wiring test that $M_c\theta$ still fires when
equation (4) is off.
...
9. **FTA directional solidification is a geometry, not a science result.**
   The Bridgman field and the two-seed bicrystal are on
   `alloy_dendrite_growth` (@sec-ad-fta). No competing-dendrite campaign
   was run; the bicrystal is one $\phi$ with two frozen orientations, not
   a two-order-parameter grain boundary. Periodic boundaries are still
   periodic — there is no no-flux mould wall from PR #103.
10. **No paper-scale melt-pool / CMS 2026 replication.** That remains
    unattempted. Do not read @sec-ad-fta as one.
