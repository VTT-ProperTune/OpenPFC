...
HIP Green-operator twins (`openpfc_homogenize_hip`,
`openpfc_inverse_homogenize_hip`) are in this application: device FFT +
Eyre–Milton, with homogeneous \(C_{11}\) matching CPU (job 21967095).
Stage 8 3-D timings below are **CPU/MPI on LUMI-C `standard`**. Coupled
dendrite device Green remains issue #157.
...
loops. Grid, ranks, loads per step (6), wall time per iteration, and
`VmHWM` are printed by `openpfc_inverse_homogenize`. The 3-D timings below
are CPU/MPI on `standard`. HIP Green is a one-GCD homogenization/inverse
parity check (job 21967095), not a 3-D GPU scaling curve.
...
- Re-entrant honeycomb on this grid was **not** auxetic; only rotating squares
  were.
- HIP Green exists for this app; it is not a 3-D GPU scaling curve.
  Coupled dendrite device Green remains #157.
