...
3. Cross-link new guides — Add a row to [`README.md`](README.md) (*Where to go first* and the right topic table—Guides by topic or Tutorials), list new walkthroughs in [`tutorials/README.md`](tutorials/README.md), and wire from [`quickstart.md`](quickstart.md) *Next steps* and/or [`getting_started/README.md`](getting_started/README.md) when the audience is learners. For discoverability, add a line to the [`faq.md`](faq.md) *Documentation map* if the page answers a common “where do I find…?” question.
