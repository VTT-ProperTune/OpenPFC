...
This page lists the **JSON / TOML surface** consumed by the default **CPU spectral** stack (`SimulationSession<SpectralCPUStack>` / ETD sessions) for shipped apps (`apps/tungsten`, `apps/aluminumNew`). **Apps may add keys** or nest objects; treat shipped inputs as ground truth when in doubt.
...
Object key `"plan_options"` (JSON) or `[plan_options]` (TOML). Passed to HeFFTe plan construction on the CPU spectral path (`SpectralCPUStack`) **and** on GPU spectral sessions (`GPUSpectralStack` via `create_cuda` / `create_hip`). **`backend: "cuda"`** is rejected for `SpectralCPUStack` / `CPUFFT` — use CPU `fftw` there, or a GPU session (`tungsten_cuda`, `aluminum_etd_cuda`, session-matrix GPU JSON). GPU overlays start from cuFFT / rocFFT defaults and apply the same reshape / pencil / GPU-aware keys.
...
| `restart_from` | `CheckpointService::restore_from_config` | Path to a published bundle (`…/step_N`). Restores fields, `Time` increment/time, result counter, method identity, and session BC state. Grid, method, or BC configuration mismatch is a hard error. Moving boundaries require saved front state; see the [tungsten restart workflow](../../apps/tungsten/README.md#restart-a-moving-front). |
