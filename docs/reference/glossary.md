...
Short definitions for OpenPFC and PFC simulation vocabulary. Deeper design detail is in [`architecture.md`](../concepts/architecture.md) and [`halo_exchange.md`](../concepts/halo_exchange.md). Use this page as a **hub**: other guides link here for jargon; follow links out to tutorials and ADRs.
...
| GPU-aware MPI | MPI implementation that can pass device buffers; required for some CUDA/HIP + HeFFTe paths — see [`INSTALL.LUMI.md`](../hpc/INSTALL.LUMI.md), [`tutorials/gpu_app_quickstart.md`](../tutorials/gpu_app_quickstart.md). |
