...
Short **if-this-then-that** pages for production-style failures. Deep background stays in [`troubleshooting.md`](../troubleshooting.md) and [`mpi_io_layout_checklist.md`](../hpc/mpi_io_layout_checklist.md).
...
| GPU backend | CUDA/HIP toolkit on PATH; GPU HeFFTe prefix matches [`gpu_path_decision.md`](../hpc/gpu_path_decision.md). |

## NaNs or diverging step

| Check | Action |
|-------|--------|
| `dt` too large for explicit pieces | Reduce `dt`; see [`science_numerics_limits.md`](../science/numerics_limits.md). |
| Debug builds | NaN checks may abort with a useful message — see [`debugging.md`](../development/debugging.md). |

## See also

- [`hpc_operator_guide.md`](../hpc/operator_guide.md) — index of HPC docs  
