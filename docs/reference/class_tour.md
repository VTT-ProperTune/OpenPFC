...
| `pfc::data::Field<T, MemorySpace>` | M2 canonical field container; owns grid data with halo padding and residency tracking (CPU/GPU) | `openpfc/kernel/data/grid_field.hpp` | `apps/heat3d/`, `apps/wave2d/`, `examples/07_array.cpp` |
| `pfc::data::field_from_subdomain*` | Factory functions to create `Field` from decomposition geometry | `openpfc/kernel/field/field_factory.hpp` | `apps/heat3d/src/cpu/heat3d_fd.cpp`, `apps/wave2d/` |
