...
| **Domain geometry** | `BinaryWriter::set_domain` and `BinaryReader::set_domain` **fail closed** before marking the domain valid: global and local dimensions must be **positive**, offsets **non-negative**, and `offset[i] + local[i] <= global[i]` on every axis. Invalid triples throw `std::invalid_argument` via `pfc::mpi::validate_subarray_domain` ([`domain_geometry.hpp`](../../include/openpfc/kernel/mpi/domain_geometry.hpp)). |
