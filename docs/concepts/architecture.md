...
- The flagship multi-rank heat example is `examples/15_finite_difference_heat.cpp` (separated halos + explicit heat equation). The padded-brick layout is showcased by `apps/heat3d/heat3d_fd_manual` — a laboratory-style FD driver that spells out the stencil and the comm/compute overlap; the compact `apps/heat3d/heat3d_fd` is the production reference. For halo design, policies, overlap, and persistent MPI options, see `docs/halo_exchange.md`.
