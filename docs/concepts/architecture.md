...
| `kernel/decomposition` | Decomposition (value-owning: `m_global_world` is a `World` by value, not a reference, so factories returning a Decomposition by value are safe — covered by `tests/unit/kernel/decomposition/test_decomposition_lifetime.cpp`), decomposition_neighbors, sparse_vector, exchange, halo_pattern, halo_mpi_types, `halo_policy.hpp`, `halo_face_layout.hpp`, `halo_exchange.hpp` (`HaloExchanger`), `separated_halo_exchange.hpp` (`SeparatedFaceHaloExchanger`), `halo_persistent.hpp` (`PersistentHaloExchanger`); decomposition_factory. |
