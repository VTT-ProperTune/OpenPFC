...
Use a padded `pfc::data::Field<T>` + `pfc::comm::HaloExchange` (or the device `HaloExchange<CUDASpace/HIPSpace>`) for **classical FD** with a single contiguous `(nx+2hw)*(ny+2hw)*(nz+2hw)` array and negative halo indexing. This is the **default** for FD-only apps and is the lowest-overhead path: ghost width is baked into storage, and the inner stencil reads `u(i±hw, j, k)` directly with no face-buffer indirection.

Use an unpadded `pfc::data::Field<T>` (`storage_halo == 0`) + `pfc::SparseHaloExchanger<T>` (typically built via `pfc::halo::make_structured_halos`) for:
...
`HaloExchange` / `SparseExchange` are **transport**: they move ghost faces given buffers and a decomposition. They do not interpret integrator stage flags or boundary-condition timing. The unpadded in-place `HaloExchanger` is deleted. The public `PaddedHaloExchanger` name is gone (Faces lives as `HaloExchange`).
...
| InPlace | *(removed)* Unpadded driver that overwrote outermost owned cells | — | Use a padded `Field` + `pfc::comm::HaloExchange` |
| **Padded** | Single contiguous `(nx+2hw)×(ny+2hw)×(nz+2hw)` buffer with a real ghost ring; owned core at `[hw, hw+n)` per axis (`pfc::data::Field` with `storage_halo > 0`) | No (FFT does not see padded layout) | `pfc::comm::HaloExchange` Faces + manual stencil over padded `pfc::data::Field<T>`; see `apps/heat3d/src/cpu/heat3d_fd_scratch.cpp`, `heat3d_fd_manual.cpp`, and `heat3d_fd.cpp` (`halo.exchange()` or `start()` / `finish()` + `pfc::gradient::FDGradient` + `pfc::field::for_each`) |
...
| Halo patterns | `halo_pattern.hpp` | `create_send_halo`, `create_recv_halo`, `create_halo_patterns`. Recv indices target in-place positions in `[0, nx·ny·nz)` today. **Fail-closed:** when `halo_width > 0`, each direction-active owned axis must be `>= halo_width` (inactive axes unchecked so flat Axes2D / `nz==1` stay valid); otherwise throws `std::invalid_argument` naming extents, direction, and `halo_width`. Recv also rejects negative/OOB local coords instead of casting them to `size_t`. |
| SparseVector + gather/scatter | `sparse_vector.hpp`, `sparse_vector_ops.hpp` | Pack path for halos. Gather/scatter fail closed on out-of-range indices with `std::runtime_error` on CPU and on CUDA/HIP device paths (`gather_cuda_impl`/`scatter_cuda_impl` and HIP twins). |
| MPI exchange | `exchange.hpp` | `isend_face` / `irecv_face` (derived types), pack `isend_data` / `irecv_data`, etc. SparseVector element counts and packed CUDA/HIP face MPI counts fail closed via `pfc::mpi::ensure_mpi_int_count` (`mpi_io_helpers.hpp`): counts above `INT_MAX` throw `std::overflow_error` before any `MPI_Send`/`Recv`/`Isend`/`Irecv` post (same class of guard as VTK local-point / BinaryWriter MPI-IO counts). |
| Device SparseVector MPI | `runtime/gpu/exchange_gpu.hpp` | `SparseVector<CUDATag\|HIPTag>` overloads (vendor shims `exchange_cuda.hpp` / `exchange_hip.hpp`). Non-blocking `isend_data` / `irecv_data` require GPU-aware MPI (`OpenPFC_MPI_CUDA_AWARE` / `OpenPFC_MPI_HIP_AWARE` and runtime `MPIX_Query_cuda_support()` / `MPIX_Query_hip_support()`); otherwise they throw `std::runtime_error` (never a silent no-op with `MPI_REQUEST_NULL`). Blocking `send_data` / `receive_data` / `send` host-stage via `cudaMemcpy` / `hipMemcpy` when unaware (same runtime gate as `DeviceFacesHalo`). Device memcpy failures throw via `pfc::cuda::detail::cuda_check` / `pfc::hip::detail::hip_check` before MPI proceeds. |
| Face MPI types | `halo_mpi_types.hpp` | `create_face_types_6` for send/recv subarrays in a single `[nx,ny,nz]` buffer. **Fail-closed:** when `halo_width > 0`, each local extent must be `>= halo_width` so a face slab fits (same rule as padded types; allows flat Axes2D domains with e.g. `nz==1`). Does **not** impose an unpadded Field's `> 2*hw` interior rule. Shared `create_face_type` also rejects non-fitting subarray starts/sizes with `std::invalid_argument`. |
| Padded face MPI types | `padded_halo_mpi_types.hpp` | `create_padded_face_types_6` — same idea but the outer extents are `(nx+2hw, ny+2hw, nz+2hw)` and recv subarrays target the dedicated halo ring rather than the outermost owned cells. **Fail-closed:** when `halo_width > 0`, each owned extent must be `>= halo_width` so the owned send slab fits; throws `std::invalid_argument` naming owned extents and `halo_width` before MPI. Does **not** impose an unpadded Field `> 2*hw` interior rule (padded Fields allow tiny owned cores). |
| Padded brick driver | `comm_halo_exchange.hpp` | `pfc::comm::HaloExchange` Faces: in-place non-blocking face exchange on a padded `pfc::data::Field<T>` so `u(i±hw, j, k)` legitimately reaches the ghost ring. **Face-only** — corners/edges are not filled. Host implementation is `comm::detail::HostFacesHalo`. Multi-field `exchange()` / `finish()` posts every bound field then one `MPI_Waitall`. |
| Padded brick, **host buffer** (full 26-direction) | `comm_halo_exchange.hpp` | `pfc::comm::HaloExchange` Full: host 26-direction fill — **3 widening passes** (X → Y-with-X-halos → Z-with-XY-halos) so every halo cell (6 faces + 12 edges + 8 corners) equals the periodic-equivalent neighbour value. Self-periodic axes (proc-grid extent 1) use host pack/unpack of widened slabs (never MPI-to-self). Default direction set is `Full3D()`. Implementation is `comm::detail::HostFullHalo`. Verified by `test_comm_halo_exchange_modes.cpp` on 1/2/4 ranks plus `hw=2`. |
| Padded brick, **device buffer** (axis-aligned 6-face) | `comm_halo_exchange_gpu.hpp` | `pfc::comm::HaloExchange<CUDASpace/HIPSpace>` Faces. Implementation is `pfc::gpu::DeviceFacesHalo` (vendor headers are thin includes). Multi-field `exchange()` posts every bound field then one `MPI_Waitall` before unpack. Same MPI face derived types as host Faces, but the base pointer is a **device** allocation. Default GPU-aware transport is **pack-to-contiguous + device-pointer MPI**; `OPENPFC_CUDA_USE_SUBARRAY_HALO=1` / `OPENPFC_HIP_USE_SUBARRAY_HALO=1` restores derived-type MPI. Without GPU-aware MPI, **pack/unpack face slabs** (`src/openpfc/runtime/gpu/padded_halo_faces.cu` / `.hip`) + MPI on pinned host. CUDA pack kernels live in `openpfc_gpu_kernels` (device-linked via `CUDA_RESOLVE_DEVICE_SYMBOLS`); HIP pack kernels live in `openpfc_hip_kernels`. Env **`OPENPFC_CUDA_FORCE_PACKED_HALO=1`** / **`OPENPFC_HIP_FORCE_PACKED_HALO=1`** forces the packed path. **Periodic faces whose neighbor rank equals the caller** (e.g. ±Z when the MPI process grid is one cell thick in Z) use **device pack/unpack** instead of GPU-aware **or packed MPI** to self — important when **local nz = 1**, because ±Z face slabs are **nx×ny** elements (~128 MiB per message at 4096²) and must not be staged through **`MPI_Irecv`/`Isend` to self**. **`kobayashi_fd_cuda`** with **one MPI rank** skips this in the timestep loop and applies **periodic halos on device** (`kobayashi_periodic_halos_xy_cuda` in `apps/kobayashi/`) instead of MPI + `cudaStreamSynchronize` / `cudaDeviceSynchronize` per exchange. **Fills only the 6 face halo strips — corners and edges are *not* populated**, so this exchanger is correct for axis-aligned stencils (5/7-point Laplacians, gradients along x/y/z) and **incorrect for any stencil that reads diagonal neighbours** such as the mixed second derivatives `u_xy` / `u_xz` / `u_yz`. Use `DeviceFullHalo` (below) for general second-order PDE kernels. |
| Padded brick, **device buffer** (full 26-direction) | `comm_halo_exchange_gpu.hpp` | `pfc::comm::HaloExchange<CUDASpace/HIPSpace>` Full. Implementation is `pfc::gpu::DeviceFullHalo` (vendor headers are thin includes). Corner- and edge-correct **multi-field** halo exchange built on the same MPI derived types and the same `padded_pack_face_kernel` as `DeviceFacesHalo`, but in **3 widening passes** (X → Y-with-X-halos → Z-with-XY-halos) so every cell of the halo ring `[-hw, 0)` and `[n, n+hw)` on every axis is populated, including the 12 edge strips and 8 corner cubes. After the call, every padded cell equals the **periodic-equivalent global value** at that offset — verified bit-identically by `tests/integration/scenarios/parallel_scaling/test_full_padded_device_halo.cpp` (CUDA) and `test_full_padded_device_halo_hip.cpp` (HIP) on 1/2/4 ranks. Self-axis loops (proc-grid extent 1 along an axis) use device pack/unpack with widened slabs and the **correct** periodic direction (the +axis halo receives the rank's *first* `hw` owned cells, not its *last* `hw`); `DeviceFacesHalo` uses the same opposite-face self-neighbor semantics for its six face slots (corners/edges still require this full exchanger). Cost: **3 syncs per call** vs. 2 for the single-pass exchanger; total MPI message count is unchanged. Env **`OPENPFC_CUDA_FORCE_PACKED_HALO=1`** falls back to a per-field axis-aligned `DeviceFacesHalo` (corners *not* filled), useful as a sanity check against the same env switch in the older exchanger. **Foundation layer for the planned PDE-kernel codegen path** (`f(t, u, ∂_i u, ∂_i ∂_j u)`): see the [archived 0.2 plans](../archive/README.md) for the historical note. |
...
| Sparse driver | `comm_sparse_exchange.hpp` | `pfc::comm::SparseExchange`: fully sparse, grid-agnostic non-blocking exchanger. Host implementation is `comm::detail::HostSparseHalo`. Accepts any `std::vector<halo::RemoteHalo<T>>` (peer + send/recv indices + tags). For structured exchanges use `pfc::halo::make_structured_halos<T>(decomp, rank, hw, dirs = Axes3D())` and `pfc::halo::copy_to_face_layout` to refill the array-of-six face buffers consumed by the templated periodic-separated FD Laplacians. |
...
- Unit: `tests/unit/kernel/decomposition/test_halo_pattern.cpp`, `test_halo_face_layout.cpp`, `test_halo_mpi_types.cpp` (non-padded `create_face_types_6` thin-domain throw, borderline owned==hw, flat `nz==1`), `test_padded_halo_mpi_types.cpp` (per-face subarray geometry, MPI_Sendrecv on `MPI_COMM_SELF`, thin owned throw + borderline owned==hw, overflow of padded extent).
...
- Integration: `test_halo_patterns.cpp`, `test_halo_exchange_driver.cpp`, `test_comm_halo_exchange.cpp` (1/2/4-rank Faces wrap, hw=2, split-phase overlap), `test_comm_halo_exchange_modes.cpp` (host full 26-direction fill on 1/2/4 ranks, edges + corners, hw=2), `test_fd_heat_mpi.cpp` — MPI parity (in-place vs separated where applicable).
- Integration (CUDA): `test_full_padded_device_halo.cpp` — `HaloExchange<CUDASpace>` Full 26-direction fill on 1, 2 (`2x1x1`), and 4 (`2x2x1`) ranks; every padded cell, including all 12 edges and 8 corners, is checked against `hash(periodic_global_coord)`. Single-rank `hw=2` and Full+Axes3D face-only cases also covered.
- Integration (HIP): `test_full_padded_device_halo_hip.cpp` — HIP twin (`HaloExchange<HIPSpace>` Full). Compiling under HIP is an M3 gate; executing on a GPU is M-LUMI.
...
| `Full3D()`  |  26  | axes + 12 edges + 8 corners                  | 3D mixed second derivatives; default for `HaloExchange` Full and `DeviceFullHalo`. |
...
If `per_rank` is provided, the exchanger calls `per_rank(rank)` for its own rank and uses that result; otherwise it uses the uniform `dirs`. Exchangers that historically defaulted to a different connectivity (`HaloExchange` Full / `DeviceFullHalo` ⇒ `Full3D()`) keep their old default after the change. Custom sets that mix faces with diagonals are tolerated by face-only exchangers (the diagonals are silently ignored — they cannot be expressed as one of the 6 canonical face slots); for full corner/edge fill use `pfc::comm::HaloExchange` Full (host or device) and feed it `Full3D()` (or a smaller preset to subset its widening passes).

Host Faces `HaloExchange` uses the zero-copy MPI subarray fast path **iff** every face slot is in the active set; subsetting via direction set falls back to the gather/scatter pack path. `DeviceFacesHalo` and `HaloExchange` on device skip excluded slots in both their GPU-aware and packed-fallback branches; same-rank periodic faces *inside* the active set still use device pack/unpack (no MPI-to-self) — this is the lever that turns off the `nx*ny*hw` ±Z self transfers when `local nz == 1`.

`DeviceFullHalo` and host Full `HaloExchange` share the same `axis_active` / `axis_widen` interpretation of diagonal directions:
...
> that variable is `0`. **Follow-up:** CUDA/HIP `HaloExchange` Faces/Full
> dirs/selector
...
- Face-only vs full-26 on the **CPU** padded-brick path: `HaloExchange` Faces remains the axis-aligned 6-face default. Full corner/edge fill is `HaloConnectivity::Full` (3-pass widening, host twin of `DeviceFullHalo`). Wiring mixed seconds into `FDGradient` / `apply_tensor_d` is still a follow-up after corners are proven.
...
- **Padded brick layout, laboratory style** (in-place ghost ring + comm/compute overlap):
  `apps/heat3d/src/cpu/heat3d_fd_manual.cpp` — `mpirun -np P ./apps/heat3d/heat3d_fd_manual N n_steps dt` runs the same heat equation against padded `pfc::data::Field<double>` + `pfc::PaddedHaloExchanger<double>` with `HeatModel::rhs` and the `for_each_inner / for_each_border / for_each_owned` lambda iterators. The driver shows the explicit non-blocking overlap (`start_halo_exchange` → `for_each_inner_omp` → `finish_halo_exchange` → `for_each_border` → Euler) and per-section `pfc::runtime::tic/toc` timers; see [`apps/heat3d/README.md`](../../apps/heat3d/README.md) for the side-by-side comparison with `heat3d_fd_scratch` and the compact `heat3d_fd`.
