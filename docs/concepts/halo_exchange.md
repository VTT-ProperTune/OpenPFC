...
| Face MPI types | `halo_mpi_types.hpp` | `create_face_types_6` for send/recv subarrays in a single `[nx,ny,nz]` buffer. **Fail-closed:** when `halo_width > 0`, each local extent must be `> 2*halo_width` (same inequality as `LocalField`); otherwise throws `std::invalid_argument` naming extents and `halo_width` before any MPI call. Shared `create_face_type` also rejects non-fitting subarray starts/sizes with `std::invalid_argument`. |
