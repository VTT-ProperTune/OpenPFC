...
| Face MPI types | `halo_mpi_types.hpp` | `create_face_types_6` for send/recv subarrays in a single `[nx,ny,nz]` buffer. **Fail-closed:** when `halo_width > 0`, each local extent must be `>= halo_width` so a face slab fits (same rule as padded types; allows flat Axes2D domains with e.g. `nz==1`). Does **not** impose `LocalField`'s `> 2*hw` interior rule. Shared `create_face_type` also rejects non-fitting subarray starts/sizes with `std::invalid_argument`. |
| Padded face MPI types | `padded_halo_mpi_types.hpp` | `create_padded_face_types_6` — same idea but the outer extents are `(nx+2hw, ny+2hw, nz+2hw)` and recv subarrays target the dedicated halo ring rather than the outermost owned cells. **Fail-closed:** when `halo_width > 0`, each owned extent must be `>= halo_width` so the owned send slab fits; throws `std::invalid_argument` naming owned extents and `halo_width` before MPI. Does **not** impose the LocalField `> 2*hw` rule (`PaddedBrick` allows tiny owned cores). |
