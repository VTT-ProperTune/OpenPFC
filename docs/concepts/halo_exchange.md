...
| Padded brick driver | `comm_halo_exchange.hpp` | `pfc::comm::HaloExchange` Faces: in-place non-blocking face exchange on a padded `pfc::data::Field<T>` so `u(i±hw, j, k)` legitimately reaches the ghost ring. **Face-only** — corners/edges are not filled. Host implementation is `comm::detail::HostFacesHalo`. |
