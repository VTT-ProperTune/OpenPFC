...
- **Model and Simulator:** use **`pfc::get_world(model)`**, **`pfc::get_fft(model)`**, and for orchestration **`pfc::get_model(sim)`**, **`pfc::get_time(sim)`**, **`pfc::get_world(sim)`**, **`pfc::get_fft(sim)`** (see `simulation/model.hpp` and `simulation/simulator.hpp`). Members remain for backward compatibility.
