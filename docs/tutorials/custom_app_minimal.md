...
Your model must construct with **`(pfc::FFT &fft, const pfc::World &world)`** — see **`pfc::Model`** in **`openpfc/kernel/simulation/model.hpp`**.
...
public:
  explicit MyModel(pfc::FFT &fft, const pfc::World &world) : pfc::Model(fft, world) {}
