...
If you want to build your own config-driven application, go to [`custom_app_minimal.md`](custom_app_minimal.md). It is intentionally about wiring: out-of-tree CMake, MPI setup, `make_simulation_session` / `pfc::sim::run`, JSON on disk and the boundary between framework plumbing and your physics. The parameter-validation details are in [`../user_guide/parameter_validation.md`](../user_guide/parameter_validation.md), and the JSON-to-session lifecycle is described in [`../user_guide/app_pipeline.md`](../user_guide/app_pipeline.md).
