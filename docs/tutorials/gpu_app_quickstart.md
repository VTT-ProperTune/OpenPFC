...
- Separate build directory per variant (e.g. `build-gpu` vs `build-cpu`) — see [`../build_cpu_gpu.md`](../hpc/build_cpu_gpu.md).
...
HIP uses `-DOpenPFC_ENABLE_HIP=ON` and ROCm-aware HeFFTe; see [`INSTALL.md`](../../INSTALL.md) §9 and [`INSTALL.LUMI.md`](../hpc/INSTALL.LUMI.md) for Cray/ROCm clusters.
...
Binary names and availability: [`../applications.md`](../user_guide/applications.md). If `tungsten_cuda` is missing, the configure step did not enable CUDA or did not find the toolkit.

## GPU-aware MPI (optional, clusters)

Device buffers may require GPU-aware MPI and environment variables (e.g. `MPICH_GPU_SUPPORT_ENABLED=1` on some Cray stacks). `App` can log hints when compile-time support is enabled — see `include/openpfc/frontend/ui/app.hpp` and [`INSTALL.LUMI.md`](../hpc/INSTALL.LUMI.md).

## Your own `App` project

The CMake pattern in [`custom_app_minimal.md`](custom_app_minimal.md) is unchanged: link `OpenPFC` built with GPU options. Consumers must use a GPU-enabled install prefix and compatible `nlohmann_json`. Validation and `model.params` behave like CPU ([`../parameter_validation.md`](../user_guide/parameter_validation.md)).

## See also

- [`../example_run_output.md`](../reference/example_run_output.md) — log shape for `App` runs  
- [`../configuration.md`](../user_guide/configuration.md) — config sections  
