...
- When an integrator does carry irreducible cross-step state, expose it as a `pfc::checkpoint::ComponentPayload` via the capture/restore helpers in [`openpfc/kernel/checkpoint/state_capture.hpp`](../../include/openpfc/kernel/checkpoint/state_capture.hpp). Explicit Euler / Heun remain empty (`empty_component_payload`). Accepted field state uses `FieldPayload`; driver-owned `Time` stays outside component payloads. See [`checkpoint_state_capture.md`](checkpoint_state_capture.md).
