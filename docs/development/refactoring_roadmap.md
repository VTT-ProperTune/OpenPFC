...
- Deprecation hygiene: `DiscreteField` (legacy type, replaced by `pfc::data::Field` from <openpfc/kernel/data/grid_field.hpp>) member `interpolate` equivalence test suppresses Clang/MSVC warnings; `Model::get_field()` Doxygen expanded for migration and out-of-tree overrides.
