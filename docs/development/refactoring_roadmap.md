...
- **GPU FFT factories:** `runtime/gpu/fft_gpu.hpp` / `fft_gpu.cpp` own `create_cuda` / `create_hip`. `runtime/common/heffte_gpu_r2c_layout.hpp` shares MPI rank/size helpers, HeFFTe box conversion, default r2c layout boxes, and MPI/decomposition mismatch checks. Vendor `fft_cuda.hpp` / `fft_hip.hpp` are thin includes.
