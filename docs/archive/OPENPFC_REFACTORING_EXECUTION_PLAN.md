...
**2026-09-03 consolidation iteration (after the M12 deletions, before the 0.2.0 tag).** A review of the branch against the audit found the duplication pattern surviving one layer up: six ETD driver classes (host/device × plain/mean-field/moving-frame), four app sessions hand-rolling IC/BC/writer wiring, `RealField` (`std::vector<double>`) still the currency of modifiers/writers/checkpoint, and 8 `[[deprecated]]` constructors holding `const Decomposition&`. All four were folded: one `SpectralETDSystem<Physics, MemorySpace>` with device-capable pointwise functors (aluminum and the plain path no longer evaluate the nonlinearity on the host), one `pfc::ui::SpectralETDSession<Physics, Stack>` on the modifier/writer catalogs with `CheckpointService` on every memory space, `FieldOutput` / `FieldView` at the boundary with `pfc::Field` = `data::Field`, one MPI-collective checkpoint publisher, zero deprecated symbols. Details in `CHANGELOG.md` ("Architecture consolidation before 0.2.0"). Verification state before this iteration: LUMI HIP Release 59/59 at HEAD (job 21689929) and a fresh CPU-only Debug build 39/39 (job 21689955); GitHub Actions had never run on `refactor-0.2` because the workflows trigger only on `master` / `develop` pushes and PRs to `master`; Tohtori CUDA was last green on 2026-09-02, before the final Gen-1 / `World` deletions, so CUDA at HEAD is untested. Verification of the consolidated tree (CPU Debug, LUMI HIP, CUDA on Tohtori) is recorded below as it lands; nothing here is ticked without a run.
...
M-LUMI still owns Cray GPU-aware MPI. Multi-rank HIP Full halo and device
SparseExchange host-stage when `MPI_HIP_AWARE=OFF` (jobs 1618752 / 1618762,
including allen_cahn/wave2d CPU-vs-HIP).
...
* [x] Create shared geometry header `kernel/decomposition/halo_geometry.hpp`: face/edge/corner slab specs, `opposite_slot`, tag allocation scheme (deterministic per-field/per-direction tag block replacing hand-spaced app tags) — single source for the 4–6 current re-implementations. (`halo_directions.hpp` includes it. Device Faces/Full call `pfc::halo::opposite_slot`. CUDA consumers of this header: not testable on LUMI.)
...
* [x] Backend-template `RemoteHalo`/`SparseHaloExchanger` → `pfc::comm::SparseExchange<MemorySpace>`, using the existing device gather/scatter, eliminating the per-step full-field D2H in `apps/allen_cahn/src/cuda/allen_cahn.cpp:100–116` and wave2d GPU. **Facade landed** (`comm_sparse_exchange.hpp` + `comm_sparse_exchange_gpu.hpp`). Public host name is `comm::detail::HostSparseHalo`. Allen–Cahn/wave2d CUDA and HIP use `SparseExchange` on device Fields.
* [x] Migrate all exchanger consumers: heat3d, wave2d (CPU+GPU), allen_cahn (CPU+GPU), kobayashi (CPU, CUDA — onto library batching; HIP — onto the device path for the first time), `FDCPUStack`, `StagePreparationService`, gpu_validation tests. **Production FD apps and `FDCPUStack`/`StagePreparationService` are on the new names. Host leftover tests and device twins (`test_full_padded_device_halo*`, `test_padded_device_halo_self_wrap*`) now use `HaloExchange`. `HaloExchangeOptions::selector` is on the host and device facades. Host and device Faces `exchange()` post every field then one `MPI_Waitall`. Host Faces/Full/persistent leftover headers are inlined into `comm_halo_exchange.hpp`.**
...
* [x] Host Faces, Full, and persistent MPI (`HostFacesHalo`, `HostFullHalo`, `HostPersistentFaces`) are inlined into `comm_halo_exchange.hpp`. `padded_halo_exchange.hpp`, `full_padded_halo_exchange.hpp`, and `halo_persistent.hpp` are deleted. Vendor CUDA/HIP thin twins of the M3 padded/full device headers are deleted. `kernel/decomposition/halo_exchange.hpp` (in-place `HaloExchanger`) is deleted. Public names are `comm::detail::HostFacesHalo` / `HostFullHalo` / `HostPersistentFaces` and `gpu::DeviceFacesHalo` / `DeviceFullHalo`.
...
* [x] Generalize state: steppers accept `std::vector<Scalar>` and host field state (`Scalar = double` or `std::complex<double>`). **All seven single-field leaves plus MultiEuler / MultiExplicitRK / MultiImex / MultiETD1 take `Scalar`. Host `Field` overloads are `HostFieldState<F, Scalar>`; multi-field leaves also take `HostFieldPack`. Mixed-scalar packs: `PackedEulerStepper<Rhs, Scalars...>`. Vector path remains.**
...
* [x] `fd_stencils.hpp` `detail::EvenFDStencil1d` / `fd_even_order_lookup` back-compat shims deleted (unused). **`StageContext` and workspace each have one type (sim names are aliases). `IntegratorMethod` in `time.hpp` is deleted.** `ImexEulerComposer::attempt` returns `StepAttemptResult`; solve extras are `last_solve_*`.
...
* [x] Define the physics concepts in `kernel/simulation/physics_concepts.hpp`: (a) field declaration (`declare_fields(SimulationState&)`) plus `add_declared_field` / `FieldDeclaration`; (c) `PointwiseRhs` (`rhs(t, G)`) and `SpectralDiagonalPhysics` (`linear_symbol(k)` + `nonlinearity(psi)`). (b) `HasParameters` nested type; declarative `ParameterSchema` is the next task.
* [x] **2026-09-03:** the host/device and per-shape ETD drivers that grew out of this task (`SpectralMeanFieldETDSystem`, `MovingFrameMeanFieldETDSystem`, `Device*` twins) are folded back into one `SpectralETDSystem<Physics, MemorySpace>`; physics supply a device-capable `pointwise()` functor. Original task text follows.
* [x] Implement `kernel/simulation/spectral_etd_system.hpp`: host driver owns work fields in `SimulationState`, transform choreography (forward → filter/N̂ → ETD combine → backward) via `IHostFFT`, `for_each_kpoint` operator preparation, `ETD1Stepper` integration, optional 2/3-rule dealias. Device `IDeviceFFT` path is `DeviceSpectralETDSystem` in `runtime/gpu/spectral_etd_system_gpu.hpp` (HIP/CUDA tests vs host ≤1e-10).
...
* [x] *(2026-09-03: `MovingFrameMeanFieldETDSystem` and its device twin are folded into `SpectralETDSystem<AluminumPhysics, MemorySpace>`; `seed_grid_fcc` is a catalog `FieldModifier`; session is `pfc::ui::SpectralETDSession` via `aluminum_session.hpp`.)* Rebuild `apps/aluminumNew` on `SpectralETDSystem`: physics = temperature-gradient/moving-frame terms + `P_F` kernel + free-energy observable (uses the M7 reduction hook); one `ParameterSchema` replacing the 130-line hand-rolled JSON block (`Aluminum.hpp:359–438`); ETD weights now from the shared cache (its inline `opL/opN` formulas retired — validated against the golden trajectory). **Host:** `MovingFrameMeanFieldETDSystem` + `aluminum_etd` / `AluminumETDSession`. Schema round-trip; operator/`N`/FE pins; one-step / 10-step / \(G_{\mathrm{grid}}\) vs Gen-1 ≤1e-10. Gen-1 still built.
* [x] Aluminum gains CUDA/HIP execution via the skeleton (previously CPU-only); add CPU-vs-GPU parity tests mirroring tungsten's. `DeviceMovingFrameMeanFieldETDSystem` + `AluminumETDGPUSession`; binaries `aluminum_etd_cuda` / `aluminum_etd_hip`. Tests `CUDA_MovingFrameETD` / `CUDA_AluminumETD` (HIP twins). HIP execute is M-LUMI.
...
* [x] Verify zero non-adapter consumers of Gen-1 `Model` / `Simulator` / A1 / A2: headers and tests deleted (`fb0d2889`). A0 `pfc::World` deleted (`490193d8`).
...
* [x] Write `docs/MIGRATION_0.1_to_0.2.md` from `0.2_migration_map.md` (removed symbol → replacement). A0 `World` listed as deleted.
* [x] Make clang-tidy blocking in CI for the new-architecture directories; keep the bidirectional layering check; add the installed-package smoke test to the release checklist. **clang-tidy job added to `ci.yml` (Unit Tests). `packaging-smoke` (`find_package` consumer) already in `ci.yml`. Layering: `scripts/check_kernel_no_frontend_includes.sh`.**
...
* [x] `kernel/simulation/model.hpp` (virtual `Model`), `model_free_functions.hpp`, `model_field_registry.hpp`. **2026-09-03:** `model_types.hpp` (`RealField` alias) is deleted too; `FieldModifier::apply` takes `field::FieldOutput<double>` and `pfc::Field<T, MemorySpace>` aliases `data::Field`.
* [x] Gen‑1 `Simulator` internals superseded by `SimulationDriver` (`simulator.hpp` and satellite dispatch headers deleted).
* [x] Adapter **A0** (`pfc::World` alias + `world::create` forwarders + `Decomposition` subworld shim). **A1** (`compat/legacy_model_physics`) and **A2** (`step_with_physics`) deleted with their tests.
* [x] Any remaining `[[deprecated]]` symbols, `OPENPFC_SUPPRESS_LEGACY_WARNINGS`, and migration-only CMake toggles. **2026-09-03:** the 6 halo + 2 stack deprecated constructors are deleted; `grep -rn "\[\[deprecated" include/ src/ apps/ examples/` is empty; `OPENPFC_SUPPRESS_LEGACY_WARNINGS` has no occurrences.
