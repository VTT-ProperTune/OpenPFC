...
**AB‑6: Stepper protocol fragmentation.** The seven leaves (Euler, RK2/RK3 Heun, ExplicitRK, EmbeddedRK, ImexEuler, Etd1) now return `StepAttemptResult` / `MultiStepAttemptResult`; in-place `step()` is attempt+commit. Remaining: on-hold `IntegratorResult`, virtual `IntegratorBase` wrapping only Euler/RK2, two `StageContext` types with "do not confuse" warnings (`step_attempt.hpp:27–29`), two workspace types, and two method-identity enums (`IntegratorMethod` stranded in `time.hpp:252` vs `RKIntegratorMethod`). Multi-field arity is inconsistently capped (`MultiStageFunction` fixed to 2 fields; `MultiEtd1Stepper` hard-requires N==2; `MultiEulerStepper` any N). All steppers operate on raw `std::vector<double>` — the elegant `state_concepts.hpp` is consumed only by tests. *Verdict: protocol port done; remaining merge is Field state, N-field packs, and duplicate context/workspace/enum.*
