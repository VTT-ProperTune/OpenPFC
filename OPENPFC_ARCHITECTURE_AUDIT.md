...
**AB‑6: Stepper protocol fragmentation.** The seven leaves return `StepAttemptResult` / `MultiStepAttemptResult`; in-place `step()` is attempt+commit. `IntegratorBase` and the on-hold result DTO are deleted. Remaining: two `StageContext` types (`step_attempt.hpp:27–29`), two workspace types, and two method-identity enums (`IntegratorMethod` in `time.hpp:252` vs `RKIntegratorMethod`). `MultiStageFunction` / `MultiEtd1Stepper` are N-ary. All seven single-field leaves accept host `Field<double>` via `vec()`; the vector path remains. Complex fields and N-field packs are still open. *Verdict: protocol port done; remaining merge is Field state on the other leaves, and duplicate context/workspace/enum.*
