...
**AB‑5: The physics interface conflates physics with numerical scheme, storage, and wiring.** `Model::step(t)` still hard-codes the integrator inside Gen-1 physics; tungsten `step()` still owns FFT + nonlinearity + ETD combine. M7: `physics_concepts.hpp`, `ParameterSchema`, host `SpectralEtdSystem`, device `DeviceSpectralEtdSystem`, A1/A2. Three-way Swift–Hohenberg toy landed (one 80-line header; Gen-1 / point-wise / spectral-ETD). *Verdict: M7 skeleton is in place; Gen-1 `Model` stays until M12 after tungsten/aluminum migrate.*
