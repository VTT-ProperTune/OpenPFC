...
**AB‑6: Stepper protocol fragmentation.** Five coexisting step APIs (in-place `step()`; IMEX `attempt/commit`; ETD1 `attempt_step` + `candidate()`; embedded-RK `attempt` with reference-member results; the *intended* shared `StepAttemptResult`), plus a sixth on-hold `IntegratorResult`, a virtual `IntegratorBase` wrapping only Euler/RK2, two `StageContext` types with "do not confuse" warnings (`step_attempt.hpp:27–29`), two workspace types, and two method-identity enums (`IntegratorMethod` stranded in `time.hpp:252` vs `RKIntegratorMethod`). Multi-field arity is inconsistently capped (`MultiStageFunction` fixed to 2 fields; `MultiEtd1Stepper` hard-requires N==2; `MultiEulerStepper` any N). All steppers operate on raw `std::vector<double>` — the elegant `state_concepts.hpp` is consumed only by tests. *Verdict: redesign/consolidate onto the attempt/commit protocol before more leaves accrete.*
