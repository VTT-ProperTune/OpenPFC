...
**AB‑5: The physics interface conflates physics with numerical scheme, storage, and wiring.** `Model::step(t)` still hard-codes the integrator inside Gen-1 physics; tungsten `step()` still owns FFT + nonlinearity + ETD combine. M7: `physics_concepts.hpp`, `ParameterSchema`, and host `SpectralEtdSystem` (FFT + `for_each_kpoint` + `Etd1Stepper` on `SimulationState`). Device `IDeviceFFT` path, observable reduction, and the three-way toy remain. A1 (`LegacyModelPhysics`) and A2 (`step_with_physics`) are in. *Verdict: host skeleton and adapters landed; device path still required before Gen-1 `Model` can go.*

**AB‑6: Stepper protocol fragmentation.** The seven leaves return `StepAttemptResult` / `MultiStepAttemptResult`; in-place `step()` is attempt+commit. `IntegratorBase` and the on-hold result DTO are deleted. `pfc::sim::StageContext` aliases `pfc::integrator::StageContext`; `StageWorkspace<T>` aliases `integrator::Workspace<T>`. `Time` stores `RKIntegratorMethod` (the `time.hpp` method enum is gone). `MultiStageFunction` / `MultiEtd1Stepper` are N-ary and accept `Scalar` (default `double`). All seven single-field leaves accept host `Field<double>` via `vec()` and `Scalar` / `Field<Scalar>` (IMEX included). MultiEuler / MultiExplicitRK / MultiImex / MultiEtd1 take `Scalar` packs. The vector path remains. Host field overloads use `HostFieldState`; multi-field leaves take `HostFieldPack`. Mixed-scalar Euler is `PackedEulerStepper`. Device ETD combine is `apply_etd1_update_{cuda,hip}`. *Verdict: protocol port done. Mixed-scalar Euler is `PackedEulerStepper`. M6 leftover: mixed-scalar ETD/IMEX/RK packs if needed. M7 started: `physics_concepts.hpp`.*
