...
**AB‑5: The physics interface conflates physics with numerical scheme, storage, and wiring.** `Model::step(t)` still hard-codes the integrator inside Gen-1 physics; tungsten `step()` still owns FFT + nonlinearity + ETD combine. M7: `physics_concepts.hpp` (`DeclaresFields`, `PointwiseRhs`, `SpectralDiagonalPhysics`) and `ParameterSchema` (parse + `format_config_error` validation + docs table). `SpectralEtdSystem` and A1/A2 remain. *Verdict: concept + schema landed; skeleton and adapters still required before Gen-1 `Model` can go.*
