...
**AB‑5: The physics interface conflates physics with numerical scheme, storage, and wiring.** `Model::step(t)` still hard-codes the integrator inside Gen-1 physics; tungsten `step()` still owns FFT + nonlinearity + ETD combine. M7: `physics_concepts.hpp`, `ParameterSchema`, host `SpectralEtdSystem`, device `DeviceSpectralEtdSystem`, A1/A2. Observable reduction and the three-way toy remain. *Verdict: host+device ETD skeleton and adapters landed; observables and three-way toy still required before Gen-1 `Model` can go.*
