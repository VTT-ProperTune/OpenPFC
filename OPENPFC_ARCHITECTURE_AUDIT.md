...
**AB‑5: The physics interface conflates physics with numerical scheme, storage, and wiring.** `Model::step(t)` still hard-codes the integrator inside Gen-1 physics; tungsten `step()` still owns FFT + nonlinearity + ETD combine. M7 concepts landed in `physics_concepts.hpp`: `DeclaresFields` / `add_declared_field`, `PointwiseRhs`, `SpectralDiagonalPhysics` (`linear_symbol` + `nonlinearity`). `ParameterSchema`, `SpectralEtdSystem`, and A1/A2 remain. *Verdict: concept surface started; skeleton and adapters still required before Gen-1 `Model` can go.*
