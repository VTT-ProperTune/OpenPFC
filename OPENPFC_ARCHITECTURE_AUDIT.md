...
**AB‑6: Stepper protocol fragmentation.** The seven leaves return `StepAttemptResult` / `MultiStepAttemptResult`; in-place `step()` is attempt+commit. `IntegratorBase` and the on-hold result DTO are deleted. `pfc::sim::StageContext` is an alias of `pfc::integrator::StageContext`. Remaining: two workspace types. `Time` stores `RKIntegratorMethod` (the `time.hpp` method enum is gone). `MultiStageFunction` / `MultiEtd1Stepper` are N-ary. All seven single-field leaves accept host `Field<double>` via `vec()`; the vector path remains. Complex fields and N-field packs are still open. *Verdict: protocol port done; remaining merge is Field state on the other leaves, and the duplicate workspace.*
