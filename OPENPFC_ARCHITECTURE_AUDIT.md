...
**AB‑5: The physics interface conflates physics with numerical scheme, storage, and wiring.** `Model::step(t)` still hard-codes the integrator inside Gen-1 physics; tungsten `step()` still owns FFT + nonlinearity + ETD combine. M7: `physics_concepts.hpp`, `ParameterSchema`, host `SpectralEtdSystem`, device `DeviceSpectralEtdSystem`, A1/A2. The three-way toy remains. *Verdict: host+device ETD skeleton, adapters, and observable reduction landed; three-way Swift–Hohenberg toy still required before Gen-1 `Model` can go.*
