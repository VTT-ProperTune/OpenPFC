...
data::Field<double, HostSpace> make_field(int n, int hw) {
  auto domain = domain::create({n, n, n});
  auto decomp = decomposition::create(domain, 1);
  return data::field_from_subdomain<double>(decomp, /*rank=*/0, hw);
}
...
TEST_CASE("for_each yields Int3 in row-major order over every owned cell",
  ...
  for (int k = 0; k < u.nz(); ++k)
    for (int j = 0; j < u.ny(); ++j)
      for (int i = 0; i < u.nx(); ++i) values_match &= u(i, j, k) == i + 10 * j;
...
TEST_CASE("for_each_coords fills every owned cell from physical coordinates",
  ...
  field::for_each_owned(u, [&](int i, int j, int k) {
    ...
    values_match &= u(i, j, k) == Catch::Approx(x + 2.0 * y + 3.0 * z);
  });
  REQUIRE(values_match);
}
...
// Tests for data::Field overloads (Primary API)
TEST_CASE("data::Field for_each_owned visits every owned cell exactly once",
          "[field][brick_iteration][field_api]") {
  auto u = make_field(4, /*hw=*/1);
  std::set<std::tuple<int, int, int>> seen;
  field::for_each_owned(u, [&](int i, int j, int k) { seen.insert({i, j, k}); });
  const auto size = u.local_size();
  REQUIRE(seen.size() == static_cast<std::size_t>(size[0] * size[1] * size[2]));
  REQUIRE(*seen.begin() == std::tuple{0, 0, 0});
  REQUIRE(*seen.rbegin() == std::tuple{size[0] - 1, size[1] - 1, size[2] - 1});
}

TEST_CASE("data::Field for_each yields Int3 in row-major order over every owned cell",
          "[field][brick_iteration][field_api]") {
  auto u = make_field(4, /*hw=*/1);
  std::vector<pfc::Int3> seen;
  field::for_each(u, [&](const auto &idx) { seen.push_back(idx); });
  ...
  REQUIRE(seen.size() == static_cast<std::size_t>(size[0] * size[1] * size[2]));
  REQUIRE(seen.front() == pfc::Int3{0, 0, 0});
  REQUIRE(seen.back() == pfc::Int3{size[0] - 1, size[1] - 1, size[2] - 1});
  ...
  // Confirm the body can write through field(idx) without any (i, j, k)
  // unpacking — this is the workflow the heat3d_fd driver will use.
  field::for_each(u, [&](const auto &idx) { u(idx) = idx[0] + 10 * idx[1]; });
  bool values_match = true;
  ...
  for (int k = 0; k < local_size[2]; ++k)
    for (int j = 0; j < local_size[1]; ++j)
      for (int i = 0; i < local_size[0]; ++i) values_match &= u(i, j, k) == i + 10 * j;
  REQUIRE(values_match);
}

TEST_CASE("data::Field for_each_inner stays in [r, n-r) and obeys r=0 -> entire owned",
          "[field][brick_iteration][field_api]") {
  auto u = make_field(5, /*hw=*/2);
  int count_r0 = 0, count_r1 = 0, count_r2 = 0;
  field::for_each_inner(u, 0, [&](int, int, int) { ++count_r0; });
  field::for_each_inner(u, 1, [&](int, int, int) { ++count_r1; });
  field::for_each_inner(u, 2, [&](int, int, int) { ++count_r2; });
  const auto size = u.local_size();
  REQUIRE(count_r0 == size[0] * size[1] * size[2]);
  REQUIRE(count_r1 == 3 * 3 * 3);
  REQUIRE(count_r2 == 1);
}

TEST_CASE("data::Field for_each_inner is a no-op when n <= 2*r",
          "[field][brick_iteration][field_api]") {
  auto u = make_field(4, /*hw=*/2);
  int count = 0;
  field::for_each_inner(u, 2, [&](int, int, int) { ++count; });
  REQUIRE(count == 0);
}

TEST_CASE("data::Field for_each_border covers owned-minus-inner exactly once",
          "[field][brick_iteration][field_api]") {
  bool all_regions_are_valid = true;
  for (int n : {4, 5, 6, 8}) {
    for (int r : {1, 2}) {
      if (n <= 2 * r) continue;
      auto u = make_field(n, /*hw=*/r);
      std::set<std::tuple<int, int, int>> border;
      bool border_is_unique = true;
      field::for_each_border(u, r, [&](int i, int j, int k) {
        const auto added = border.emplace(i, j, k).second;
...
TEST_CASE("data::Field for_each_border falls back to the whole owned region when n <= 2*r",
          "[field][brick_iteration][field_api]") {
  auto u = make_field(4, /*hw=*/2);
  int count = 0;
  field::for_each_border(u, 2, [&](int, int, int) { ++count; });
  const auto size = u.local_size();
  REQUIRE(count == size[0] * size[1] * size[2]);
}

TEST_CASE("data::Field stencil over inner region only reads owned cells (no halo dependency)",
          "[field][brick_iteration][field_api]") {
  auto u = make_field(6, /*hw=*/1);
  const auto size = u.local_size();
  for (int k = 0; k < size[2]; ++k) {
    for (int j = 0; j < size[1]; ++j) {
      for (int i = 0; i < size[0]; ++i) u(i, j, k) = i + 10 * j + 100 * k;
    }
  }
...
TEST_CASE("data::Field for_each_owned_omp visits same set as serial counterpart",
          "[field][brick_iteration][field_api]") {
  auto u = make_field(4, /*hw=*/0);
  const auto size = u.local_size();
  std::vector<int> hits(size[0] * size[1] * size[2], 0);
  field::for_each_owned_omp(u, [&](int i, int j, int k) {
    const std::size_t lin = static_cast<std::size_t>(i) +
                            static_cast<std::size_t>(j) * size[0] +
                            static_cast<std::size_t>(k) * size[0] * size[1];
    hits[lin] = 1;
  });
  bool all_cells_visited = true;
  for (int v : hits) all_cells_visited &= v == 1;
...
TEST_CASE("data::Field for_each_coords fills every owned cell from physical coordinates",
          "[field][brick_iteration][field_api]") {
  auto u = make_field(3, /*hw=*/1);
  bool values_match = true;
  field::for_each_coords(
      u, [](double x, double y, double z, double &v) { v = x + 2.0 * y + 3.0 * z; });
  field::for_each_owned(u, [&](int i, int j, int k) {
