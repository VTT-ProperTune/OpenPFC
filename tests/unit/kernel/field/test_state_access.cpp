...
TEST_CASE("FieldView const access", "[field][state_access]") {
  ...
  SECTION("is_compatible_with returns correct bool") {
    ...
    FieldView<double> incompatible_extent(data.data(), data.size(),
    ...
    FieldView<double> incompatible_spacing(data.data(), data.size(), extents,
                                           different_spacing, origin);
    REQUIRE_FALSE(view.is_compatible_with(incompatible_spacing));

    pfc::types::Real3 different_origin{1.0, 0.0, 0.0};
    FieldView<double> incompatible_origin(data.data(), data.size(), extents, spacing,
                                          different_origin);
...
TEST_CASE("Field aliasing detection", "[field][state_access]") {
  ...
  FieldView<double> input(input_data.data(), input_data.size(), extents, spacing,
                          origin);
  ...
  SECTION("aliasing detected when storage overlaps") {
    ...
    FieldView<double> shared_view(shared_data.data(), shared_data.size(), extents,
                                  spacing, origin);
    FieldOutput<double> shared_output(shared_data.data(), shared_data.size());

    // Should throw when storage aliases
    REQUIRE_THROWS_AS(shared_output.validate_no_alias(shared_view),
  ...
  SECTION("aliasing validation through validate_no_alias function") {
    ...
    REQUIRE_THROWS_AS(validate_no_alias(shared_output, shared_view),
                      std::invalid_argument);
  }
}
...
TEST_CASE("Shape compatibility validation", "[field][state_access]") {
  ...
  SECTION("incompatible extents throw") {
    ...
    FieldView<double> field3(data3.data(), data3.size(), different_extents, spacing,
                             origin);

    REQUIRE_THROWS_AS(validate_shape_compatibility(field1, field3),
                      std::invalid_argument);
  }
  ...
  SECTION("incompatible spacing throws") {
    ...
    FieldView<double> field3(data3.data(), data3.size(), extents, different_spacing,
                             origin);
  ...
  SECTION("incompatible origin throws") {
    pfc::types::Real3 different_origin{1.0, 0.0, 0.0};
    std::vector<double> data3(64, 1.0);
    FieldView<double> field3(data3.data(), data3.size(), extents, spacing,
                             different_origin);
...
TEST_CASE("FieldBundle multi-field", "[field][state_access]") {
  ...
  FieldView<double> lap_view(lap_data.data(), lap_data.size(), extents, spacing,
                             origin);

  SECTION("construction with multiple fields") {
    FieldBundle<FieldView<double>, FieldView<double>, FieldView<double>> bundle(
  ...
  SECTION("validate_shapes() detects incompatible fields") {
    ...
    FieldView<double> w_view(w_data.data(), w_data.size(), different_extents,
                             spacing, origin);
...
TEST_CASE("FieldView with complex types", "[field][state_access]") {
  ...
  FieldView<std::complex<double>> view(data.data(), data.size(), extents, spacing,
                                       origin);
...
TEST_CASE("FieldOutput with complex types", "[field][state_access]") {
  ...
  SECTION("write semantics for complex types") {
    ...
    for (std::size_t i = 0; i < output.size(); ++i) {
      mutable_ptr[i] =
          std::complex<double>(static_cast<double>(i), static_cast<double>(i) * 2.0);
    }
...
TEST_CASE("StageContext MPI coordination fields", "[field][state_access]") {
  ...
  REQUIRE(req.needs_boundary_update);
  REQUIRE_FALSE(req.needs_halo_exchange);
