...
field::PaddedBrick<double> make_brick_domain(int n, int hw) {
  auto world =
...
TEST_CASE("for_each_owned visits every owned cell exactly once (Domain version)",
          "[field][brick_iteration][domain][unit]") {
  auto u = make_brick_domain(4, /*hw=*/1);
...
TEST_CASE("for_each yields Int3 in row-major order over every owned cell (Domain version)",
          "[field][brick_iteration][domain][unit]") {
  auto u = make_brick_domain(4, /*hw=*/1);
  ...
  for (int k = 0; k < u.nz(); ++k)
    for (int j = 0; j < u.ny(); ++j)
      for (int i = 0; i < u.nx(); ++i)
        values_match &= u(i, j, k) == i + 10 * j;
  REQUIRE(values_match);
}

TEST_CASE("for_each_inner stays in [r, n-r) and obeys r=0 -> entire owned (Domain version)",
          "[field][brick_iteration][domain][unit]") {
  auto u = make_brick_domain(5, /*hw=*/2);
...
TEST_CASE("for_each_inner is a no-op when n <= 2*r (Domain version)",
          "[field][brick_iteration][domain][unit]") {
  auto u = make_brick_domain(4, /*hw=*/2);
  int count = 0;
  field::for_each_inner(u, 2, [&](int, int, int) { ++count; });
  REQUIRE(count == 0);
}

TEST_CASE("for_each_border covers owned-minus-inner exactly once (Domain version)",
          "[field][brick_iteration][domain][unit]") {
  ...
  for (int n : {4, 5, 6, 8}) {
    for (int r : {1, 2}) {
      if (n <= 2 * r)
        continue;
      auto u = make_brick_domain(n, /*hw=*/r);
      ...
      for (const auto &c : border)
        regions_are_disjoint &= !inner.contains(c);
      ...
      for (const auto &b : border) {
        ...
        const bool on_face = (i < r) || (i >= n - r) || (j < r) ||
                             (j >= n - r) || (k < r) || (k >= n - r);
...
TEST_CASE("for_each_border falls back to the whole owned region when n <= 2*r (Domain version)",
          "[field][brick_iteration][domain][unit]") {
  auto u = make_brick_domain(4, /*hw=*/2);
  int count = 0;
...
TEST_CASE("stencil over inner region only reads owned cells (no halo dependency) (Domain version)",
          "[field][brick_iteration][domain][unit]") {
  auto u = make_brick_domain(6, /*hw=*/1);
  for (int k = 0; k < u.nz(); ++k) {
    for (int j = 0; j < u.ny(); ++j) {
      for (int i = 0; i < u.nx(); ++i)
        u(i, j, k) = i + 10 * j + 100 * k;
    }
  }
...
TEST_CASE("for_each_owned_omp visits same set as serial counterpart (Domain version)",
          "[field][brick_iteration][domain][unit]") {
  auto u = make_brick_domain(4, /*hw=*/0);
  ...
  for (int v : hits)
    all_cells_visited &= v == 1;
  REQUIRE(all_cells_visited);
}

TEST_CASE("for_each_coords fills every owned cell from physical coordinates (Domain version)",
          "[field][brick_iteration][domain][unit]") {
  auto u = make_brick_domain(3, /*hw=*/1);
  bool values_match = true;
...
TEST_CASE("PaddedBrick operator[] matches operator() for Int3 (Domain version)",
          "[field][brick_iteration][domain][unit]") {
  auto u = make_brick_domain(3, /*hw=*/1);
