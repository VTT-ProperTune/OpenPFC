...
/**
 * @file fd_gradient_domain.cpp
 * @brief Domain-based version: unit tests for `pfc::field::FdGradient<G>` covering
 *        the first- and second-derivative branches and the constructor's diagnostic
...
Field<double> make_field_domain(int N, int hw) {
  ...
  return pfc::data::field_from_subdomain_unpadded<double>(decomp, /*rank=*/0,
                                                          hw);
}

} // namespace

TEST_CASE("FdGradient<GradXYZ> recovers exact first derivatives on a linear field (Domain version)",
          "[kernel][field][fd_gradient][domain][unit]") {
...
TEST_CASE("FdGradient<ValueXXX> populates value, x, and xx coherently (Domain version)",
          "[kernel][field][fd_gradient][domain][unit]") {
...
TEST_CASE("FdGradient ctor throws when a model needs a missing D1 table (Domain version)",
          "[kernel][field][fd_gradient][domain][unit]") {
...
TEST_CASE("FdGradient ctor throws when a model needs a missing D2 table (Domain version)",
          "[kernel][field][fd_gradient][domain][unit]") {
...
TEST_CASE("FdGradient ctor throws when halo_width is below stencil half_width (Domain version)",
          "[kernel][field][fd_gradient][domain][unit]") {
  ...
  using namespace pfc;
  auto world =
      domain::create_world(GridSize({N, N, N}), PhysicalOrigin({0.0, 0.0, 0.0}),
                           GridSpacing({1.0, 1.0, 1.0}));
  auto decomp = decomposition::create(world, 1);
  field::PaddedBrick<double> brick(decomp, /*rank=*/0, hw);
  REQUIRE_THROWS_AS((pfc::gradient::FDGradient<OnlyXX>(brick, order)),
                    std::invalid_argument);
}

TEST_CASE("FdGradient<OnlyX>: order-2 D1 is exact on a constant field "
          "(anti-symmetry) and on a linear ramp (closed form) (Domain version)",
          "[kernel][field][fd_gradient][domain][unit]") {
  const int order = 2;
  const int hw = 1;
  const int N = 8;
...
TEST_CASE("pfc::gradient::FDGradient binds to a PaddedBrick and matches the "
          "LocalField path (Domain version)",
          "[kernel][field][fd_gradient][domain][unit]") {
  ...
  using namespace pfc;
  auto world =
      domain::create_world(GridSize({8, 8, 8}), PhysicalOrigin({0.0, 0.0, 0.0}),
                           GridSpacing({1.0, 1.0, 1.0}));
  auto decomp = decomposition::create(world, 1);
...
TEST_CASE("FDGradient prepare invokes callback when provided (Domain version)",
          "[kernel][field][fd_gradient][domain][unit]") {
...
TEST_CASE("FDGradient prepare is no-op without callback (Domain version)",
          "[kernel][field][fd_gradient][domain][unit]") {
...
TEST_CASE("FDGradient PaddedBrick constructor accepts callback (Domain version)",
          "[kernel][field][fd_gradient][domain][unit]") {
...
TEST_CASE("FDGradient raw-pointer constructor accepts callback (Domain version)",
          "[kernel][field][fd_gradient][domain][unit]") {
...
TEST_CASE("halo_preparation calls callback (Domain version)",
          "[kernel][field][fd_gradient][domain][unit]") {
  int callback_invoked = 0;
  auto callback = [&callback_invoked]() { ++callback_invoked; };
...
TEST_CASE("halo_preparation and prepare identity (Domain version)",
          "[kernel][field][fd_gradient][domain][unit]") {
  int callback_invoked = 0;
  auto callback = [&callback_invoked]() { ++callback_invoked; };
