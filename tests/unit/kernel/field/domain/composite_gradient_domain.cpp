...
/**
 * @file composite_gradient_domain.cpp
 * @brief Domain-based version: validates the multi-field composite per-point evaluator.
 *
 * @details
...
TEST_CASE("CompositeGradient fans two heterogeneous FD evaluators into a "
          "single composite per-point view (Domain version)",
          "[kernel][field][composite_gradient][domain][unit]") {
  ...
  auto u = pfc::data::field_from_subdomain_unpadded<double>(decomp, /*rank=*/0,
                                                            /*halo_width=*/1);
