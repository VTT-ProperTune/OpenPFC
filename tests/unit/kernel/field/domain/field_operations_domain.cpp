...
TEST_CASE("field::apply sets constant value over inbox (Domain version)",
          "[field_ops][domain][unit]") {
...
TEST_CASE("field::apply_with_time uses time parameter (Domain version)",
          "[field_ops][domain][unit]") {
...
TEST_CASE("field::apply_inplace modifies field based on current value (Domain version)",
          "[field_ops][domain][unit]") {
  ...
  // Double all values
  field::apply_inplace(model, "psi",
                       [](const Real3 & /*x*/, double current) { return 2.0 * current; });
...
TEST_CASE("field::apply_inplace selective update preserves untouched cells (Domain version)",
          "[field_ops][domain][unit]") {
...
TEST_CASE("field::apply_inplace_with_time uses time parameter (Domain version)",
          "[field_ops][domain][unit]") {
...
TEST_CASE("legacy adapter wraps lambda into FieldModifier (Domain version)",
          "[field_ops][domain][unit]") {
  auto world = domain::create_world(GridSize({8, 1, 1}),
                                    PhysicalOrigin({0.0, 0.0, 0.0}),
                                    GridSpacing({1.0, 1.0, 1.0}));
