...
TEST_CASE("field_from_subdomain: parity with LocalField (halo=0) (Domain version)",
...
TEST_CASE("field_from_subdomain: parity with LocalField for multiple ranks (Domain version)",
          "[field_factory][domain][unit]") {
...
TEST_CASE("field_from_subdomain: parity with PaddedBrick (halo=n) (Domain version)",
          "[field_factory][domain][unit]") {
  const int nx = 8, ny = 6, nz = 4;
  auto world = domain::create_world({nx, ny, nz});
  auto decomp = decomposition::create(world, 1);
...
TEST_CASE("field_from_subdomain: parity with PaddedBrick for multiple ranks (Domain version)",
          "[field_factory][domain][unit]") {
  const int nx = 12, ny = 8, nz = 6;
  auto world = domain::create_world({nx, ny, nz});
...
TEST_CASE("field_from_subdomain: rejects negative halo (Domain version)",
          "[field_factory][domain][unit]") {
  const int nx = 8, ny = 6, nz = 4;
  auto world = domain::create_world({nx, ny, nz});
  auto decomp = decomposition::create(world, 1);
...
TEST_CASE("field_from_subdomain: geometry matches decomposition (Domain version)",
          "[field_factory][domain][unit]") {
  const int nx = 16, ny = 12, nz = 8;
  auto domain_obj = domain::with_spacing({nx, ny, nz}, {1.5, 2.0, 2.5});
  auto decomp = decomposition::create(domain_obj, 4);
...
TEST_CASE("field_from_subdomain: creates valid field with varying sizes (Domain version)",
          "[field_factory][domain][unit]") {
  auto domain_obj = domain::create({4, 4, 4});
  auto decomp = decomposition::create(domain_obj, 1);
