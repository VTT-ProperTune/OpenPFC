...
TEST_CASE("test_composite_gradient_box3i_domain",
  ...
  // Create composite using Box3i+Domain factory
  Box3i region = Box3i::from_bounds({0, 0, 0}, {N - 1, N - 1, N - 1});
  auto composite = pfc::gradient::make_composite_gradient<TwoField>(region, domain, grad_u, grad_v);
...
TEST_CASE("test_composite_gradient_box3i_domain_spacing",
  ...
  // Create a small test field with custom spacing
  constexpr int N = 8;
  const int order = 2;
  ...
  // Create composite using Box3i+Domain+spacing factory
  Box3i region{{0, 0, 0}, {N, N, N}};
  std::array<double, 3> custom_spacing{1.0, 1.0, 1.0};
  auto composite = pfc::gradient::make_composite_gradient<TwoField>(region, domain, custom_spacing, grad_u, grad_v);
