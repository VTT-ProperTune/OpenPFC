...
pfc::decomposition::Decomposition make_decomp(int N, int nproc, double origin,
  ...
  auto local_domain = pfc::domain::create(
      ...
      pfc::GridSpacing({spacing, spacing, spacing}), {true, true, true});
...
TEST_CASE("Decomposition: factory return-by-value keeps the global World intact",
  ...
  const auto d = pfc::decomposition::domain(decomp);

  SECTION("size survives the factory's local Domain destruction") {
    const auto sz = pfc::domain::get_size(d);
    REQUIRE(sz[0] == 16);
    REQUIRE(sz[1] == 16);
    REQUIRE(sz[2] == 16);
  }
  SECTION("origin survives") {
    const auto org = pfc::domain::get_origin(d);
  ...
  SECTION("spacing survives") {
    const auto sp = pfc::domain::get_spacing(d);
...
TEST_CASE("Decomposition: copy-construction yields a self-contained copy",
  ...
  auto make_copy = []() {
    auto src_domain = pfc::domain::create({8, 8, 8});
  ...
  auto copy = make_copy();
  const auto d = pfc::decomposition::domain(copy);
  const auto sz = pfc::domain::get_size(d);
  REQUIRE(sz[0] == 8);
  REQUIRE(sz[1] == 8);
  REQUIRE(sz[2] == 8);
...
TEST_CASE("Decomposition: stored by value inside an aggregate stays valid "
  ...
  auto build_holder = []() {
    auto src_domain = pfc::domain::create(
        pfc::GridSize({12, 12, 12}), pfc::PhysicalOrigin({1.0, 2.0, 3.0}),
        pfc::GridSpacing({0.5, 0.5, 0.5}), {true, true, true});
    return Holder{pfc::decomposition::create(src_domain, /*nproc=*/1)};
  };

  auto holder = build_holder();
  const auto d = pfc::decomposition::domain(holder.decomp);
  const auto org = pfc::domain::get_origin(d);
  // Domain created with {1.0, 2.0, 3.0} origin
