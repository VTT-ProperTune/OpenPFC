...
#if !defined(OPENPFC_TEST_MOVING_FRAME_ETD_HIP) &&                                  \
    !defined(OPENPFC_TEST_MOVING_FRAME_ETD_CUDA)
...
#elif defined(OPENPFC_TEST_MOVING_FRAME_ETD_CUDA)
...
using pfc::sim::DeviceMovingFrameMeanFieldETDSystem;
...
struct MovingFrameToy {
  ...
  [[nodiscard]] double nonlinearity(double psi, double psi_mf, double p_star,
                                    double T_var) const {
    return p3 * psi * psi + q3 * psi_mf * psi_mf - p_star + T_var;
  }
...
void fill_cosine(Field<double> &psi) {
  ...
  psi.apply([&](double x, double, double) {
    return -0.006 + 0.01 * std::cos(2.0 * pfc::pi * x / lx);
  });
}
...
TEST_CASE("device MovingFrameMeanFieldETDSystem matches host within 1e-10",
          "[gpu][moving_frame_etd]") {
#if defined(OPENPFC_TEST_MOVING_FRAME_ETD_HIP)
  ...
  MovingFrameMeanFieldETDSystem<MovingFrameToy> host_sys(host_phys, cpu_fft,
                                                         host_state, dt);
  host_sys.step(0.0);

  MovingFrameToy dev_phys{};
  ...
  DeviceMovingFrameMeanFieldETDSystem<MovingFrameToy, Space> dev_sys(
      dev_phys, gpu_fft, dev_state, dt);
  REQUIRE(dev_sys.linear_symbol().size() == gpu_fft.size_outbox());
  REQUIRE(dev_sys.correlation_kernel().size() == gpu_fft.size_outbox());
