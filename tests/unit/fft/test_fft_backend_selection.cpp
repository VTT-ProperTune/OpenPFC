...
TEST_CASE("FFT Backend - parse backend from JSON (FFTW)", "[fft][backend][config][unit]") {
  json config = {
      {"backend", "fftw"}
  };
...
TEST_CASE("FFT Backend - parse backend from JSON (case insensitive)", "[fft][backend][config][unit]") {
...
TEST_CASE("FFT Backend - default to FFTW if not specified", "[fft][backend][config][unit]") {
...
#if defined(OpenPFC_ENABLE_CUDA)
TEST_CASE("FFT Backend - parse CUDA backend from JSON", "[fft][backend][config][cuda][unit]") {
...
#else
TEST_CASE("FFT Backend - CUDA backend throws if not compiled", "[fft][backend][config][unit]") {
  json config = {{"backend", "cuda"}};
...
TEST_CASE("FFT Backend - size queries work through interface", "[fft][backend][unit]") {
  auto world = world::create({16, 16, 16});
  auto decomposition = decomposition::create(world, 1);
  auto fft = fft::create_with_backend(decomposition, 0, fft::Backend::FFTW);
