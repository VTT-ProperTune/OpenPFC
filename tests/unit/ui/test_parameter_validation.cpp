...
TEST_CASE("ParameterMetadata integer validation", "[parameter_validation][unit]") {
  SECTION("Valid integer value") {
    auto meta = ParameterMetadata<int>::builder()
      .name("num_steps")
      .range(1, 1000)
      .build();
...
TEST_CASE("ParameterValidator with required parameters", "[parameter_validation][unit]") {
  ...
  SECTION("Wrong type") {
    json config = {
      {"temperature", "3300"},  // String instead of number
      {"density", -0.10}
    };
...
TEST_CASE("ParameterValidator with optional parameters", "[parameter_validation][unit]") {
  ParameterValidator validator;
  ...
  SECTION("Optional parameter provided but invalid") {
    json config = {
      {"temperature", 300.0},
      {"optional_param", 2.0}  // Out of range
    };
...
TEST_CASE("ParameterValidator with default values", "[parameter_validation][unit]") {
  ...
  SECTION("Uses default when not provided") {
    ...
    REQUIRE(result.is_valid());
    REQUIRE(result.validated_params.size() == 1);
    REQUIRE(result.validated_params["param_with_default"].find("42") != std::string::npos);
    REQUIRE(result.validated_params["param_with_default"].find("default") != std::string::npos);
  }

  SECTION("Overrides default when provided") {
    json config = {
      {"param_with_default", 100.0}
    };

    auto result = validator.validate(config);
    REQUIRE(result.is_valid());
    REQUIRE(result.validated_params["param_with_default"].find("100") != std::string::npos);
    REQUIRE(result.validated_params["param_with_default"].find("default") == std::string::npos);
  }
}
