...
- ☑ Tungsten HIP one-node strong scaling (LUMI, 2026-09-06, `tungsten_hip` 0.2.0, 768³ / 10 steps, I/O off, one rank per GCD). Median `wall_step` after `--warmup-frames=1`: 847 / 455 / 315 / 215 ms on 1 / 2 / 4 / 8 GCDs (efficiency 100% / 93% / 67% / 49%). Jobs 21759720 (`dev-g`), 21759944 (`dev-g`), 21759945 (`dev-g`), 21759946 (`standard-g`). Pins: `tests/baselines/perf/lumi-dev-g-tungsten-hip-{1,2,4}gcd-release-768.json` and `lumi-standard-g-tungsten-hip-8gcd-release-768.json`. Use median, not mean: 4-GCD step 3 stalled at 11.4 s on all ranks. HIP `fft` region timers after step 1 are not trustworthy on this path. Recipe: [`docs/hpc/lumi_gpu_scaling.md`](../../docs/hpc/lumi_gpu_scaling.md). 256³ 1-GCD was 20 ms (too small). No 1-vs-N field checksum yet.
- ☑ Tungsten HIP multi-node (same 768³ / 10 steps): 16 GCD job 21760377 median 222 ms (eff 24%, slower than 8 GCD); 32 GCD job 21760378 median 103 ms (eff 26%). Pins: `lumi-standard-g-tungsten-hip-{16,32}gcd-release-768.json`. Pencil `p2p_plined` off-node does not help this grid.
- ☑ Tungsten HIP slabs multi-node (768³ / 10 steps, `use_pencils=false`, `p2p_plined`, GPU-aware). Median `wall_step` after warmup 1: 851 / 216 / 182 / 150 / 122 ms on 1 / 8 / 16 / 24 / 32 GCDs (eff 100% / 49% / 29% / 24% / 22%). Jobs 21761281, 21761220, 21761221, 21761282, 21761283 (`standard-g`). 16 GCD is faster than 8 GCD. Pins: `lumi-standard-g-tungsten-hip-slabs-{1,8,16,24,32}gcd-release-768.json`. `SPECTRAL_CHECKSUM` 1 vs 16 ~1e-12 rel.
- ☑ Heat3D FD HIP 3D (`heat3d_fd_hip`, 512³ / 20 steps / dt=0.01 / order 2, device halo). Median `wall_step` after warmup 1: 5.91 / 0.951 / 0.567 / 0.476 / 0.394 ms on 1 / 8 / 16 / 24 / 32 GCDs (eff 100% / 78% / 65% / 52% / 47%). Jobs 21761036 (`dev-g`), 21761037–21761042 (`standard-g`). Pins: `lumi-dev-g-heat3d-fd-hip-1gcd-release-512.json`, `lumi-standard-g-heat3d-fd-hip-{8,16,24,32}gcd-release-512.json`. `HEAT3D_HIP_CHECKSUM` 1 vs 16 ~5e-15 rel.
- ☑ Tungsten HIP y-outbox slabs (768³ / 10 steps, real z-slabs + complex y-slabs, `use_pencils=false` off-node). Median `wall_step` after warmup 1: 851 / 177 / 84 / 72 / 63 ms on 1 / 8 / 16 / 24 / 32 GCDs (eff 100% / 60% / 64% / 50% / 42%). Jobs 21761281, 21764313, 21764315, 21764316, 21764317 (`standard-g`). Pins: `lumi-standard-g-tungsten-hip-slabs-{1,8,16,24,32}gcd-release-768.json` (8–32 updated). `SPECTRAL_CHECKSUM` 1 vs 16 ~1e-12 rel.
- ☑ Heat3D spectral HIP (`heat3d_spectral_hip`, 768³ / 20 steps / dt=0.01, implicit Euler, 2 FFTs/step, slabs + GPU-aware `p2p_plined`). Median `wall_step` after warmup 1: 416 / 204 / 127 / 88 / 50 / 39 / 34 ms on 1 / 2 / 4 / 8 / 16 / 24 / 32 GCDs (eff 100% / 102% / 82% / 59% / 52% / 44% / 38%). Jobs 21773962, 21774367, 21774368, 21774369, 21773966, 21773967, 21773968 (`standard-g`). Pins: `lumi-standard-g-heat3d-spectral-hip-{1,2,4,8,16,24,32}gcd-release-768.json`. `HEAT3D_SPECTRAL_HIP_CHECKSUM` 1 vs 16 ~3e-16 rel.
