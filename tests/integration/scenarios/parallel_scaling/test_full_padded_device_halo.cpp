 ...
 * @brief Exhaustive 26-direction fill for `pfc::comm::HaloExchange<CUDASpace>`
 *        (`HaloConnectivity::Full`).
 *
 * Covers 1, 2 (`2x1x1`), and 4 (`2x2x1`) ranks. Every padded cell must match
 * `hash(periodic_global_coord)` after `exchange()`.
 */
...
void fill_owned_hash(pfc::data::Field<double, CUDASpace> &u, int field) {
...
bool full_periodic_hash_matches(pfc::data::Field<double, CUDASpace> &u, int field) {
  bool ok = true;
  u.with_host_view([&](double *data, std::size_t) {
    const auto n = u.size3();
...
void run_full_halo_check(const pfc::decomposition::Decomposition &decomp, int rank,
  ...
  fill_owned_hash(v, 1);
  pfc::comm::HaloExchange<CUDASpace, double> halo({&u, &v}, decomp, rank,
                                                  MPI_COMM_WORLD, opt);
...
TEST_CASE("HaloExchange CUDASpace Full: 1-rank periodic fill (all 26 halos)",
          "[gpu][padded_halo][full_halo][cuda]") {
  int rank = 0, size = 1;
...
TEST_CASE("HaloExchange CUDASpace Full: 2-rank 2x1x1 fill (X real, Y/Z self)",
          "[MPI][gpu][padded_halo][full_halo][cuda]") {
  int rank = 0, size = 1;
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  MPI_Comm_size(MPI_COMM_WORLD, &size);
  if (size != 2) {
...
TEST_CASE("HaloExchange CUDASpace Full: 4-rank 2x2x1 fill (X+Y real, Z self)",
          "[MPI][gpu][padded_halo][full_halo][grid][cuda]") {
  int rank = 0, size = 1;
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  MPI_Comm_size(MPI_COMM_WORLD, &size);
  if (size != 4) {
...
TEST_CASE("HaloExchange CUDASpace Full: hw=2 1-rank widened halo",
          "[gpu][padded_halo][full_halo][cuda]") {
  int rank = 0, size = 1;
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  MPI_Comm_size(MPI_COMM_WORLD, &size);
...
TEST_CASE("HaloExchange CUDASpace Full+Axes3D fills only the 6 axis faces",
          "[gpu][padded_halo][full_halo][halo_directions][cuda]") {
  int rank = 0, size = 1;
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  MPI_Comm_size(MPI_COMM_WORLD, &size);
  if (size != 1) {
