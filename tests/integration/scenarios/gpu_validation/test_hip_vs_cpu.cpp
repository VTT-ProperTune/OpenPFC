...
TEST_CASE("HIP vs CPU diffusion consistency (smoke)", "[integration][gpu][hip]") {
  ...
  auto decomp_cpu = decomposition::create(domain, size);
  ...
  // Full HIP vs CPU consistency testing requires HIP spectral support
  // (OpenPFC_ENABLE_HIP_SPECTRAL) which provides rocFFT backend via
  // fft::create_hip() - not tested here
