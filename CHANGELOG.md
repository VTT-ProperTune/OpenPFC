...
- **CUDA padded halos**: `pfc::cuda::PaddedDeviceHaloExchanger` (`runtime/cuda/padded_device_halo_exchange.hpp`) and face pack/unpack kernels (`src/openpfc/runtime/cuda/padded_halo_faces.cu`, linked into **`kobayashi_fd_cuda`**) — same MPI derived types as `PaddedHaloExchanger<double>` with a **device** base pointer; GPU-aware MPI when supported, else narrow face slabs + pinned host. Env **`OPENPFC_CUDA_FORCE_PACKED_HALO=1`** forces the packed path.
