...
- `alloy_pf_directional` starting point is the locked \(12\times 3.2\,\mu\mathrm{m}\), \(W_0=10\,\mathrm{nm}\) bicrystal (default CLI / `start` / `benchmark`); last-bit CI is `repro` / `alloy-pf-directional-repro`. MPI/HIP halos use 0.2 `HaloExchange` Full2D/Full3D (`DeviceFullHalo` on HIP device pointers).
