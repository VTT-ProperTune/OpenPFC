...
/**
 * @file fd_gradient_device_gpu.hpp
 * @brief Single-source GPU per-point FD gradient evaluator (M3).
 *
 * CUDA feature set (`CompositeGradientDevice`, `evaluate_fd_grad_composite`)
 * plus the HIP padded-`Field` factory (`storage_halo`). Vendor headers
 * `fd_gradient_device.hpp` re-export these types into `pfc::cuda` /
 * `pfc::hip`.
 *
 ...
 *
 * **Usage** (typical for a device translation unit):
 *
 * @code
...
OPENPFC_INLINE_HD G evaluate_fd_grad(const FdGradientDevicePOD &e, int ix, int iy,
  ...
  // Mixed seconds: separable D1⊗D1 on the padded buffer. Order-2 (hw1=1)
  // reduces to the familiar 4-corner stencil with weight
  // (1/(2 dx))*(1/(2 dy)).
  if constexpr (pfc::field::has_xy<G>) {
    double acc = 0.0;
    for (int i = 1; i <= e.hw1; ++i) {
      const std::ptrdiff_t is = static_cast<std::ptrdiff_t>(i) * e.sx;
      for (int j = 1; j <= e.hw1; ++j) {
        const std::ptrdiff_t js = static_cast<std::ptrdiff_t>(j) * e.sy;
        acc += e.cx1[i] * e.cy1[j] *
               (e.d_core[c + is + js] - e.d_core[c - is + js] -
                e.d_core[c + is - js] + e.d_core[c - is - js]);
      }
    }
    g.xy = acc;
  ...
  if constexpr (pfc::field::has_xz<G>) {
    ...
    for (int i = 1; i <= e.hw1; ++i) {
      ...
      for (int k = 1; k <= e.hw1; ++k) {
        const std::ptrdiff_t ks = static_cast<std::ptrdiff_t>(k) * e.sz;
        acc += e.cx1[i] * e.cz1[k] *
               (e.d_core[c + is + ks] - e.d_core[c - is + ks] -
                e.d_core[c + is - ks] + e.d_core[c - is - ks]);
      }
    }
    g.xz = acc;
  ...
  if constexpr (pfc::field::has_yz<G>) {
    ...
    for (int j = 1; j <= e.hw1; ++j) {
      ...
      for (int k = 1; k <= e.hw1; ++k) {
        const std::ptrdiff_t ks = static_cast<std::ptrdiff_t>(k) * e.sz;
        acc += e.cy1[j] * e.cz1[k] *
               (e.d_core[c + js + ks] - e.d_core[c - js + ks] -
                e.d_core[c + js - ks] + e.d_core[c - js - ks]);
      }
    }
    g.yz = acc;
 ...
 *
 * @code
 * auto composite = pfc::gpu::create_composite_device<WaveLocal>(eval_u, eval_v);
 * @endcode
 */
template <class Composite, class... PerFieldGrads>
 ...
 * @param order         Even spatial order; requires
 *                      `u.storage_halo() >= order / 2`. Defaults to 2.
 *
 * @throws std::invalid_argument if `u.storage_halo() <= 0` (unpadded
