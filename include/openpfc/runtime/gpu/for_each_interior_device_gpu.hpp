...
/**
 * @file for_each_interior_device_gpu.hpp
 * @brief Single-source GPU `for_each_interior_device` for CUDA and HIP (M3).
 *
 * CUDA feature set: single-field, multi-field N=2..4, composite POD, autotune
 * hook. Vendor headers re-export into `pfc::sim::cuda` / `pfc::sim::hip`.
 *
 * GPU twin of the canonical CPU driver loop `pfc::sim::for_each_interior`.
 *
 * @details
 ...
 * @endcode
 *
 * @see openpfc/runtime/gpu/fd_gradient_device_gpu.hpp — per-point evaluator
...
__global__ void for_each_interior_device_kernel_multi(
    Model model, ::pfc::gpu::CompositeGradientDevicePOD eval, DuPack du,
  ...
  const Composite grad =
      ::pfc::gpu::evaluate_fd_grad_composite<Composite, PerFieldGrads...>(
          eval, ix, iy, iz);
...
for_each_interior_device(const Model &model,
                         const ::pfc::gpu::CompositeGradientDevicePOD &eval,
...
inline void for_each_interior_device(
    const Model &model,
    const ::pfc::gpu::CompositeGradientDevice<Composite, PerFieldGrads...> &eval,
    DevicePtrPack2 du, double t, int nx, int ny, int nz,
    gpuStream_t stream = nullptr) {
