 ...
 */
inline int checked_padded_extent_cuda(int n, int hw) {
  if (hw < 0) {
    throw std::invalid_argument(
        "CUDA PaddedDeviceHaloExchanger: halo width must be non-negative (got " +
        std::to_string(hw) + ")");
  }
...
/**
 * @brief MPI halo exchange for a padded Field-layout buffer on the CUDA device.
 *
...
public:
  ...
  PaddedDeviceHaloExchanger(const decomposition::Decomposition &decomp, int rank,
    ...
    m_nxp = detail::checked_padded_extent_cuda(nx, hw);
    m_nyp = detail::checked_padded_extent_cuda(ny, hw);
    m_nzp = detail::checked_padded_extent_cuda(nz, hw);
