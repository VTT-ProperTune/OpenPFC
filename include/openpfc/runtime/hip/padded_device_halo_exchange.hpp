 ...
 *
 * - **GPU-aware MPI** (`OpenPFC_MPI_HIP_AWARE` at compile time and
 *   `MPIX_Query_hip_support() == 1` at runtime on Open MPI): `MPI_Irecv` /
 *   `MPI_Isend` use the device pointer directly (after HIP stream sync).
 ...
 *
 * Set **`OPENPFC_HIP_FORCE_PACKED_HALO=1`** to force the fallback path.
 *
 ...
 * inter-rank** faces.
 *
 * **Profiling:** set **`OPENPFC_HIP_PROFILE_HALO=1`** to accumulate CPU-side
 * `MPI_Wtime` buckets inside `exchange_halos_device` (pre-stream sync, MPI
 * window, post HIP sync, and packed-path D2H / `MPI_Waitall` / H2D segments).
 * Call `pfc::hip::print_hip_halo_exchange_cpu_timers(comm)` on rank 0 after
 * the loop to print **`OPENPFC_HIP_PROFILE_HALO_SUMMARY`** (MPI_MAX per bucket
 * across ranks).
 *
 ...
 * `OPENPFC_HIP_PROFILE_HALO_SUMMARY`. */
inline void print_hip_halo_exchange_cpu_timers(MPI_Comm comm) {
  if (!hip_halo_exchange_perf_enabled()) {
    return;
  }
  int rank = 0;
  ...
  const double mx_post = reduce_max(T.post_exchange_hip_sync);
...
private:
   ...
   * call `hipMemcpy` here; transfers are meant to be performed by the MPI/HIP
   ...
   * `hipMemcpyAsync` calls per face — set **`OPENPFC_HIP_FORCE_PACKED_HALO=1`**
   * to force that path for performance experiments.
   */
  void exchange_gpu_aware_(double *d_padded, hipStream_t stream) {
  ...
  void exchange_packed_fallback_(double *d_padded, hipStream_t stream) {
    const bool perf = hip_halo_exchange_perf_enabled();
    auto &H = hip_halo_exchange_cpu_timers();
