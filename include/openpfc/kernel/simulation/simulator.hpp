 ...
 *
 * Post-class helpers (integrator seam, scheduled writes, non-member accessors)
 * live in **`simulator_integrator.hpp`** and **`simulator_queries.hpp`**, included
 * at the end of this header so a single `#include
 * <openpfc/kernel/simulation/simulator.hpp>` remains sufficient for callers.
 *
