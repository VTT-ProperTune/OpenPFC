 ...
 * @brief OPENPFC_PROFILE("path") { … } and PFC_PROFILE_SCOPE (see OPENPFC_PROFILING_LEVEL)
 */
...
/// Unique per source line and macro expansion (avoids nested-scope name clashes / -Wshadow).
...
/// TimerOutputs-style block: OPENPFC_PROFILE("my/region") { … }; registers path on first use.
#define OPENPFC_PROFILE(NAME_STR)                                                    \
  if (::pfc::profiling::ProfilingTimedScope OPENPFC_PROFILE_UID_(_openpfc_prof_){(NAME_STR)}; \
      true)

/// Single-statement scope (unique local name via __LINE__ / __COUNTER__).
#define PFC_PROFILE_SCOPE(name)                                                      \
  ::pfc::profiling::ProfilingTimedScope OPENPFC_PROFILE_UID_(_pfc_profile_scope_){(name)}
