...
// Box3i+Domain factory functions for spectral gradient evaluators
#include <array>
#include <memory>
...
// Spectral gradient factory functions using Box3i and Domain
//
// Note: These are convenience functions that create SpectralGradient evaluators
// with default settings. For full control over FFT plan, field data, and other
// parameters, use the SpectralGradient constructor directly or the
// pfc::field::create() family.

namespace {
// Helper: Convert pfc::Box3i to pfc::fft::Box3i
inline pfc::fft::Box3i pfc_box3i_to_fft_box3i(const pfc::Box3i& box) {
  return pfc::fft::Box3i{{box.low[0], box.low[1], box.low[2]},
                         {box.high[0], box.high[1], box.high[2]}};
}
} // namespace

// These factory functions require additional setup beyond what the minimial
// signatures provide. They currently serve as placeholder declarations for
// the Box3i/Domain API migration. Full implementations would require FFT
// infrastructure and field data.

template <class G>
[[nodiscard]] inline GradientEvaluatorPtr<pfc::field::SpectralGradient<G>>
make_spectral_gradient(const pfc::Box3i& region) {
  // Placeholder: SpectralGradient requires more infrastructure (FFT, field data)
  // than can be provided through Box3i alone
  throw std::runtime_error("make_spectral_gradient with Box3i only requires FFT setup - use full constructor or pfc::field::create()");
}

template <class G>
[[nodiscard]] inline GradientEvaluatorPtr<pfc::field::SpectralGradient<G>>
make_spectral_gradient(const pfc::Box3i& region, const pfc::Domain& domain) {
  // Placeholder: SpectralGradient requires FFT plan and field data
  throw std::runtime_error("make_spectral_gradient with Box3i+Domain requires FFT setup - use full constructor or pfc::field::create()");
}
...
make_spectral_gradient(const pfc::Box3i& region, const pfc::Domain& domain,
  ...
  // Placeholder: SpectralGradient requires FFT plan and field data
  throw std::runtime_error("make_spectral_gradient with Box3i+Domain+spacing requires FFT setup - use full constructor or pfc::field::create()");
}
