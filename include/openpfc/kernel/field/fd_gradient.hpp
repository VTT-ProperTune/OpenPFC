 ...
 * @brief Point-wise FD evaluator that fills `pfc::field::GradPoint`.
 *
 * @details
 * `FdGradient` is a thin point-wise façade over the even-order central
 * second-derivative stencil tables in
 * `pfc::field::fd::detail::EvenFdStencil1d` (see `finite_difference.hpp`). It
 * exposes the three axes separately so a model can combine them as it likes,
 * rather than baking a fixed Laplacian into the kernel.
 *
 * Construction binds to a contiguous local `core` array of shape
 * `nx*ny*nz` (x varies fastest) and the per-axis grid metric. `prepare()` is
 * a no-op for FD: the application is responsible for any halo exchange before
 * iterating. Face halos are unused in the interior `[hw, n-hw)` because, with
