...
template <typename T> class cuda_span {
public:
  ...
  constexpr cuda_span(pointer ptr, index_type count) noexcept
      : data_(ptr), size_(count) {}
  constexpr cuda_span(pointer first, pointer last) noexcept
...
namespace std {
template <typename T> using span = cuda_span<T>;
}
#endif